...
(async()=>{
 ...
 try{
  ...
  assert.equal(await portLabel('gain_ref').innerText(),'out');
  ...
  const bounds=await portLabel('gain_ref').evaluate(e=>{const a=e.getBoundingClientRect(),b=e.closest('.node').getBoundingClientRect();return {contained:a.left>=b.left&&a.right<=b.right,full:e.title};});assert.ok(bounds.contained);assert.equal(bounds.full,'out');
  checks.push('Uniform and Sampler colors follow their sources; Uniform keeps its out port after rename and Sampler shows its source name');
