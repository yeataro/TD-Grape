...
async function run(){
  ...
  try{
    ...
    assert.deepEqual(await page.locator('.toolbar .graph-tool-group').evaluateAll(groups=>groups.map(g=>[...g.querySelectorAll('button')].map(b=>b.id))),[['undo','redo'],['boxselect'],['customnames'],['code']]);
    assert.equal(await page.locator('#selectiontoolbar #graphcopy,#selectiontoolbar #graphpaste,#selectiontoolbar #graphdelete').count(),3);
    ...
    const fullscreenPreferences=await page.evaluate(()=>localStorage.getItem('sgrapeExperimentsV1'));
    assert.equal(await page.evaluate(()=>EDITOR_DEV_SETTINGS.systemClock),false);assert.equal(await page.locator('#uisystemclock').isVisible(),false);
    ...
    assert.equal(await page.locator('.footer-preferences #uisystemclock + #uifullscreen').count(),1);assert.equal(await page.locator('#uisystemclock').isVisible(),true);
    ...
    assert.equal(await page.locator('#uisystemclock').isVisible(),false);assert.equal(await page.evaluate(()=>systemClockTimer),null);
    ...
    assert.equal(await page.locator('.canvas-view-tools #uisystemclock + #uifullscreen').count(),1);assert.equal(await page.locator('#uisystemclock').isVisible(),true);
    await page.screenshot({path:path.join(folder,'fullscreen-focus-clock.png')});
    ...
    assert.equal(await page.locator('.footer-preferences #uisystemclock').count(),1);assert.equal(await page.locator('#uisystemclock').isVisible(),false);assert.equal(await page.evaluate(()=>systemClockTimer),null);
    assert.equal(await page.locator('#graphfocus').getAttribute('aria-pressed'),'true');assert.equal(await page.locator('#uifullscreen').getAttribute('aria-pressed'),'false');
    await page.locator('#graphfocus').click();assert.equal(await viewState(),originalView);
    assert.equal(await page.locator('.footer-preferences > button:last-child').getAttribute('id'),'uifullscreen');
    assert.equal(await page.evaluate(()=>chromeFullscreenButton===$('#uifullscreen')),true);
    assert.equal(await page.evaluate(()=>localStorage.getItem('sgrapeExperimentsV1')),fullscreenPreferences);
    ...
    assert.equal(await page.locator('#uisystemclock').isVisible(),false);assert.equal(await page.evaluate(()=>systemClockTimer),null);assert.equal(await page.evaluate(()=>localStorage.getItem('sgrapeExperimentsV1')),fullscreenPreferences);
    ...
    await page.evaluate(()=>setUIExperiments({systemClock:true}));const clockEnabledPreferences=await page.evaluate(()=>localStorage.getItem('sgrapeExperimentsV1'));
    await page.locator('#uifullscreen').click();await page.waitForFunction(()=>!!document.fullscreenElement);
    await page.locator('#uifullscreen').click();await page.waitForFunction(()=>!document.fullscreenElement);await settle();
    assert.equal(await page.locator('#uisystemclock').isVisible(),true);assert.equal(await page.evaluate(()=>EDITOR_DEV_SETTINGS.systemClock),true);assert.equal(await page.evaluate(()=>localStorage.getItem('sgrapeExperimentsV1')),clockEnabledPreferences);assert.equal(await viewState(),originalView);
    await page.evaluate(()=>setUIExperiments({systemClock:false}));
    checks.push('Unsupported or rejected fullscreen remains a harmless view operation with accurate button state');
    checks.push('native fullscreen shows the clock in footer and focused graph, restores both saved off/on preferences on exit, and leaves clock off after rejected entry without persisting an override');
