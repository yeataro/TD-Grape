...
(async()=>{
 ...
 try{
  ...
  const value=async v=>{if(await page.evaluate(()=>dirty))await apply();await select('Live');const input=page.locator('[data-native-source="Live"] [data-source-component="0"]');await input.fill(String(v));await input.press('Enter');await page.waitForFunction(()=>!nativeSourceBusy&&!nativeMutationBusy);await settle();assert.equal(nativeRows.Live.values[0],v);};
