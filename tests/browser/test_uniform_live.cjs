...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(async()=>{window.viewGeneration=uniformLive.generation;selectedInputId='live';render();await Promise.resolve();uniformLive.watchVisible();});
  assert.equal(await page.evaluate(()=>uniformLive.ready&&uniformLive.generation===viewGeneration),true);
  await h.settle();assert.equal(await page.evaluate(()=>uniformLive.ready&&uniformLive.generation===viewGeneration),true);
  checks.push('replacing visible controls during a full render retains the subscription while their first visibility measurement is pending');
