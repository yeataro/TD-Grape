  ...
  try{
    for(const type of ['float','int','uint','bool','vec3','ivec3','bvec3','double','dvec3','mat3','mat2x3','dmat2x3']){
      ...
      if(matrix){
        ...
        await open('inputs',type,'matrix-target','value');assert.ok(await page.evaluate(()=>creatorMatches.length>0));await compare();await compare('mat');
      }else if(!['bool','bvec3'].includes(type)){await open('inputs',type);await compare();await compare('mat');}
    }
    ...
    for(const [type,key,visible,output]of [['float','matrix_convert',true],['double','matrix_convert',true],['mat2x3','convert',true],['mat2x3','matrix_convert',true,'mat2x3'],['dmat3x2','matrix_convert',true,'mat3x2'],['vec2','matrix_convert',false],['ivec3','convert',true,'vec3']]){
      await reset({type,count:0});await open('outputs',type);
      const result=await page.evaluate(key=>{
        $('#createsearch').value='convert';renderCreator();const index=creatorMatches.findIndex(m=>m.d.key===key);
        if(index<0)return {visible:false};chooseCreator(index);const n=current().nodes.find(n=>n.id===selected);
        return {visible:true,created:definition(n)?.key===key,from:n?.params.fromType,output:ports(n,'outputs').out,edge:current().edges.at(-1),history:past.length,open:!!creatorState};
      },key);
      assert.equal(result.visible,visible,JSON.stringify({type,key,result}));
      if(!visible)continue;
      assert.equal(result.created,true);assert.equal(result.from,type);assert.equal(result.history,1);assert.equal(result.open,false);assert.deepEqual(result.edge.from,['source','out']);
      if(output)assert.equal(result.output,output);assert.equal(await page.evaluate(type=>isMatrixType(type),result.output),key==='matrix_convert');
      await page.evaluate(()=>undo());await settle();assert.equal(await page.evaluate(()=>current().nodes.length),1);assert.equal(await page.evaluate(()=>current().edges.length),0);
    }
    checks.push('scalar/matrix wire creation routes to the correct Convert family, keeps same-shape priority, rejects vector-to-matrix entry and supports one Undo');
