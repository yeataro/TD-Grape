...
async function run(){
  ...
  try{
    ...
    assert.equal(await page.locator('#about').innerText(),'このアプリについて');
    ...
    assert.deepEqual(translated,{parameter:'パラメーター',help:'ヘルプ',controls:'OP パラメーター',preview:'マテリアルプレビュー',stage:'Pixel ステージ',missing:[]});
    ...
    assert.deepEqual(await page.evaluate(()=>['panel.parameterTitle','panel.helpTitle','controls.title','preview.top','preview.material','layout.title'].map(t)),['參數','說明','OP 參數','輸出預覽','材質預覽','面板配置']);
