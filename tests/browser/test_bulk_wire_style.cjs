...
(async()=>{
 ...
 try{
  ...
  await openNodes(['b','c']);await page.locator('[data-edit=arrange]').focus();await page.keyboard.press('ArrowRight');await page.locator('#grapharrangesubmenu [data-arrange=left]').press('Enter');assert.equal(await page.evaluate(()=>current().nodes.find(n=>n.id==='b').ui.x===current().nodes.find(n=>n.id==='c').ui.x),true);
