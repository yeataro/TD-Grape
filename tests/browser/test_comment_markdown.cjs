...
async function run(){
  ...
  const parameterEntry=()=>page.locator('#inspector [data-comment-node]');
  ...
  try{
    ...
    assert.equal(await entry().isVisible(),false);assert.equal(await parameterEntry().isVisible(),true);assert.equal(await parameterEntry().inputValue(),markdown);assert.equal(await page.locator('#inspector .comment-node-preview').count(),0);assert.equal(await preview().evaluate(e=>getComputedStyle(e).backgroundColor),'rgba(0, 0, 0, 0)');assert.equal(await page.evaluate(()=>past.length),0);
    checks.push('canvas renders Markdown headings, emphasis, lists, safe links and exact fenced GLSL tokens; unknown fences stay literal, while Parameter always exposes the unformatted source textarea');
    ...
    await reset(hostile);assert.equal(await card().locator('img,script,iframe,object').count(),0);assert.equal(await preview().locator('a').count(),0);assert.equal(await parameterEntry().inputValue(),hostile);assert.ok((await preview().innerText()).includes('<img'));assert.equal(await page.evaluate(()=>window.__commentXss),undefined);
    ...
    await entry().fill('# Canvas update\n\n**Saved once**');assert.equal(await text(),'Initial **note**');await entry().press('Control+Enter');await settle();assert.equal(await entry().isVisible(),false);assert.equal(await preview().locator('h1').innerText(),'Canvas update');assert.equal(await parameterEntry().inputValue(),'# Canvas update\n\n**Saved once**');assert.equal(await page.evaluate(()=>past.length),1);assert.equal(await page.evaluate(()=>hasShaderChanges()),false);
    ...
    await parameterEntry().fill('## Parameter update');await page.locator('#fit').click();await settle();assert.equal(await parameterEntry().isVisible(),true);assert.equal(await preview().locator('h2').innerText(),'Parameter update');assert.equal(await text(),'## Parameter update');assert.equal(await page.evaluate(()=>past.length),2);assert.equal(await page.evaluate(()=>hasShaderChanges()),false);
    checks.push('canvas single click selects and Enter edits; Ctrl+Enter restores canvas reading while Parameter blur keeps its source field; synced edits Undo/Redo exactly and never dirty the shader');

    await preview().dblclick();await entry().fill('Uncommitted **draft**');await page.evaluate(()=>render());assert.equal(await entry().inputValue(),'Uncommitted **draft**');assert.equal(await entry().evaluate(e=>e===document.activeElement),true);assert.equal(await parameterEntry().inputValue(),'## Parameter update');
    ...
    await parameterEntry().fill('Another draft');await page.evaluate(()=>inspector());assert.equal(await parameterEntry().inputValue(),'Another draft');await parameterEntry().press('Escape');assert.equal(await parameterEntry().isVisible(),true);assert.equal(await parameterEntry().inputValue(),'## Parameter update');assert.equal(await text(),'## Parameter update');assert.equal(await page.evaluate(()=>past.length),2);
    ...
    const parameterGeometry=()=>page.locator('.comment-node-parameter').evaluate(section=>{const rect=e=>{const r=e.getBoundingClientRect();return{top:r.top,bottom:r.bottom,height:r.height};};return{entry:rect(section.querySelector('textarea')),hint:rect(section.querySelector('small')),body:rect(document.querySelector('#parameterbody'))};});
    const compact=await parameterGeometry(),layoutBefore=await graphJSON(),layoutHistory=await page.evaluate(()=>past.length);await page.locator('#resize-live').focus();for(let i=0;i<3;i++)await page.keyboard.press('Shift+ArrowDown');await settle();const larger=await parameterGeometry();
    assert.ok(larger.entry.height>compact.entry.height+30,'Parameter source editor expands with its pane');for(const g of [compact,larger]){assert.ok(g.entry.bottom<=g.hint.top,'hint stays below the editor');assert.ok(g.hint.bottom<=g.body.bottom+1,'hint remains visible at the panel bottom');assert.ok(g.body.bottom-g.hint.bottom<24,'hint follows the lower edge instead of floating above unused space');}
    for(let i=0;i<3;i++)await page.keyboard.press('Shift+ArrowUp');await settle();const restored=await parameterGeometry();assert.ok(Math.abs(restored.entry.height-compact.entry.height)<2,'source editor shrinks back with the pane');assert.equal(await graphJSON(),layoutBefore);assert.equal(await page.evaluate(()=>past.length),layoutHistory);
    await page.locator('[data-workspace-group="parameters"] .workspace-fold').click();assert.equal(await page.locator('#parameterbody').isVisible(),false,'full-height styling respects a collapsed Parameter panel');await page.locator('[data-workspace-group="parameters"] .workspace-fold').click();assert.equal(await parameterEntry().isVisible(),true);
    await page.locator('#inspector').getByRole('tab',{name:'設定',exact:true}).click();assert.equal(await parameterEntry().count(),0);assert.equal(await page.locator('#inspector').evaluate(e=>e.classList.contains('comment-parameters')),false);await page.locator('#inspector').getByRole('tab',{name:'參數',exact:true}).click();assert.equal(await parameterEntry().isVisible(),true);
    await page.locator('[data-node="value"] .node-title').click();assert.equal(await parameterEntry().count(),0);assert.equal(await page.locator('#inspector').evaluate(e=>e.classList.contains('comment-parameters')),false);await preview().click();assert.equal(await parameterEntry().isVisible(),true);assert.equal(await graphJSON(),layoutBefore);
    checks.push('Parameter textarea grows/shrinks with its pane and keeps the hint at the bottom; panel collapse hides it and other nodes/tabs release its fill layout without graph/history changes');
    ...
    await parameterEntry().hover();await page.mouse.wheel(0,180);await page.waitForFunction(()=>document.querySelector('#inspector [data-comment-node]').scrollTop>0);assert.equal(await page.evaluate(()=>scale),zoom);assert.equal(await page.evaluate(()=>past.length),0);
    checks.push('canvas reader/editor and the full-height Parameter textarea scroll their own long content without zooming the graph or adding history');
    ...
    await page.evaluate(()=>{readonly=true;render();});await preview().dblclick();assert.equal(await entry().isVisible(),false);await parameterEntry().focus();await parameterEntry().press('x');assert.equal(await parameterEntry().isVisible(),true);assert.equal(await parameterEntry().getAttribute('readonly'),'');assert.equal(await parameterEntry().inputValue(),'Touch **note**');assert.equal(await text(),'Touch **note**');assert.equal(await page.evaluate(()=>past.length),0);
    checks.push('touch canvas single tap reads/selects and double tap edits; readonly preserves canvas reading and a visible uneditable Parameter source field');
