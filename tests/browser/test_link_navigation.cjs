...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>{setUIExperiments({linkArrowDisplay:'always',reverseInputLinkArrowOnHover:true});setLinkLinesVisible(true);fit();});
  ...
  await page.evaluate(()=>setUIExperiments({linkArrowDisplay:'hidden'}));assert.equal(await visibleSource.count(),0);
  ...
  await page.evaluate(()=>{window.hoverGraphBefore=JSON.stringify(graph);window.hoverPastBefore=past.length;setUIExperiments({linkArrowDisplay:'hover'});fit();});
  await page.mouse.move(10,10);assert.equal(await visibleSource.count(),1);assert.equal(await visibleSource.evaluate(e=>getComputedStyle(e).opacity),'0');
  async function assertSocketEndpoints(){assert.ok(await page.evaluate(()=>[...document.querySelectorAll('#wires .wire-link')].every(path=>{
    const edge=path.edgeSelectionItem;return [['from','outputs',0],['to','inputs',path.getTotalLength()]].every(([side,kind,distance])=>{const node=current().nodes.find(n=>n.id===edge[side][0]),expected=point(node,edge[side][1],kind),actual=path.getPointAtLength(distance);return Math.hypot(actual.x-expected.x,actual.y-expected.y)<.01;});
  })));}
  await assertSocketEndpoints();await page.locator('[data-node="a"] [data-port="out"]').hover();assert.equal(await visibleSource.evaluate(e=>getComputedStyle(e).opacity),'1');await assertSocketEndpoints();
  await visibleSource.hover();assert.equal(await visibleSource.evaluate(e=>getComputedStyle(e).opacity),'1');await visibleSource.click({button:'right'});assert.deepEqual(await page.locator('#grapheditmenu [data-link-target]').evaluateAll(es=>es.map(e=>e.dataset.linkTarget)),['b','c']);await page.keyboard.press('Escape');await page.locator('#canvas').focus();await page.mouse.move(10,10);
  await incoming.hover();assert.equal(await incoming.evaluate(e=>getComputedStyle(e).opacity),'1');await page.mouse.move(10,10);assert.equal(await incoming.evaluate(e=>getComputedStyle(e).opacity),'0');
  await page.evaluate(()=>setLinkLinesVisible(false));assert.equal(await visibleSource.evaluate(e=>getComputedStyle(e).opacity),'1');assert.equal(await page.locator('#wires .wire-link').count(),0);
  await page.evaluate(()=>setLinkLinesVisible(true));assert.equal(await visibleSource.evaluate(e=>getComputedStyle(e).opacity),'0');await assertSocketEndpoints();
  assert.equal(await page.evaluate(()=>JSON.stringify(graph)===hoverGraphBefore&&past.length===hoverPastBefore),true);
  ...
  await mode.selectOption('hover');assert.equal(await page.evaluate(()=>EDITOR_DEV_SETTINGS.linkArrowDisplay),'hover');
  await page.keyboard.press('Escape');
  await page.reload();await page.waitForSelector('.node');assert.equal(await page.evaluate(()=>EDITOR_DEV_SETTINGS.linkArrowDisplay),'hover');
  checks.push('three-way selector changes and persists mode; legacy booleans migrate without overriding an explicit new choice, invalid values reset safely');
