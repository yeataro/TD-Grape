...
(async()=>{
 ...
 try{
  ...
  await page.locator('#uiexperiments').click();await page.locator('[data-experiment="showLinkLines"]').uncheck();await page.keyboard.press('Escape');
  ...
  await page.evaluate(()=>{readonly=false;render();setUIExperiments({showLinkLines:true});fit();});
  ...
  await page.screenshot({path:path.join(folder,'links.png')});assert.deepEqual(errors,[]);await h.finish();console.log(JSON.stringify({passed:true,checks}));
