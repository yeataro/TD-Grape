...
(async()=>{
  ...
  assert.equal(await page.locator('#wires path').first().evaluate(e=>getComputedStyle(e).pointerEvents),'stroke');checks.push('sidebar scrollbar colors and selectable wire strokes');
  ...
  assert.equal(await page.evaluate(async()=>{const icon=new Image();icon.src=document.querySelector('link[rel="icon"][type="image/svg+xml"]').href;await icon.decode();return icon.naturalWidth>0&&icon.naturalWidth===icon.naturalHeight;}),true);checks.push('local SVG favicon decodes under the editor Content Security Policy');
