...
(async()=>{
 ...
 try {
  ...
  assert.ok(await page.locator('#sourcecreate').isVisible());
  assert.ok((await page.locator('#sourcecreate').boundingBox()).y<(await page.locator('#nativeuniforms').boundingBox()).y);
  assert.equal(await page.locator('#nativeuniforms [data-input-source=input0]').count(),1);
  assert.equal(await page.locator('#sourcekind option[value=sampler]').evaluate(e=>e.hidden),true);
  await page.selectOption('#sourcekind','constant');await page.fill('#sourcename','cGain');await page.locator('#sourcecreate button[type=submit]').click();
  ...
  await page.selectOption('#sourcekind','top_input');await page.fill('#sourcename','Second');await page.locator('#sourcecreate button[type=submit]').click();
  ...
  await drag(page.locator('#nativeuniforms [data-input-reference="'+slot+'"]'),false);
  ...
  await page.evaluate(({x,y})=>openCreator(x,y),drop);await page.fill('#createsearch','Second');assert.ok(await page.locator('[data-create-entry="input:'+slot+'"]').count());await page.evaluate(()=>closeCreator());
  ...
  assert.deepEqual(errors,[]);console.log('Inputs creation, named constants, top references, clipboard, delete/restore, mouse and touch palette drags passed');
