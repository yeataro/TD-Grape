...
(async()=>{
  ...
  try{
    ...
    await reset();await page.evaluate(()=>{scale=.4;transform();});await drag(-80,-100);const minimumSize=await size('note');assert.equal(minimumSize.width,190);assert.ok(minimumSize.height>=30&&minimumSize.height<=70);
    ...
    await drag(500,500);assert.deepEqual(await size('note'),{width:1200,height:1200});const atMaximum=await page.evaluate(()=>past.length);await drag(30,30);assert.equal(await page.evaluate(()=>past.length),atMaximum);
