...
(async()=>{
  ...
  try{
    ...
    for(const fontScale of [1,4,10])for(const kind of ['plain','h1','code','ul','ol'])for(const appearance of ['opaque','transparent','hidden','transparent-hidden']){
      ...
      if(appearance.includes('hidden'))for(const isSelected of [false,true]){await page.evaluate(isSelected=>{document.activeElement?.blur();selection=new Set(isSelected?['note']:[]);selected=isSelected?'note':null;render();},isSelected);await page.mouse.move(1500,1000);await settle();const actual=await layout();if(isSelected)sameGeometry(measured,actual);else{sameGeometry(measured,{...actual,reader:measured.reader});assert.ok(Math.abs(measured.reader.y-actual.reader.y-measured.title.height)<.06,'hidden title frees its row above the reading area');assert.ok(Math.abs(actual.reader.height-measured.reader.height-measured.title.height)<.06,'reading area gains the hidden title height');}assert.equal(await card('note').locator('.node-title').isVisible(),isSelected);}
