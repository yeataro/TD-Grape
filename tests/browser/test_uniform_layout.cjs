 ...
 try{
  await page.evaluate(()=>{
   ...
   window.resetLayoutFixture=()=>{
    ...
    nativeSourceError='';nativeSourceUncertain=false;applyInFlight=null;stage='pixel';graphTrail=[];graph.functions=[];revision=100;editVersion=0;past=[];future=[];
    ...
    api=async(route,body)=>{
     requests.push({route,body:body&&clone(body)});
  ...
  for(const action of ['rename','type','remove']){
  ...
  checks.push('this source rename, type and removal drafts still block values, including direct request calls');
  ...
  await reset();await move();await apply();await page.evaluate(async()=>{releaseApply(true);await layoutApply;});assert.equal(await control().isEnabled(),true);
  await page.evaluate(()=>receiveNativeSources(layoutSnapshot()));assert.equal(await control().isEnabled(),true);
  checks.push('a Shader rebuild alone leaves the unchanged source editable; REST refreshes before writing and the backend checks native identity');
