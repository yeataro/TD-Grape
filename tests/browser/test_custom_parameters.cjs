...
(async()=>{
 ...
 try{
  ...
  await page.locator('[data-workspace-panel=uniforms]').click();await page.locator('[data-input-source=gain] .input-source-select').click();await page.locator('#inspector [data-source-custom]').click();await page.locator('#customdialog').waitFor({state:'visible'});await page.locator('#customdialog select').selectOption('Performance');await page.locator('#customdialog').getByRole('button',{name:'Add control',exact:true}).click();await page.locator('[data-custom-control=Urenamed]').waitFor();
