...
(async()=>{
  ...
  const tick=fraction=>page.evaluate(fraction=>{const motion=canvasMotion;for(const [id,callback]of [...cameraTest.frames]){cameraTest.frames.delete(id);callback(motion.start+motion.duration*fraction);}},fraction);
  ...
  try{
    await page.evaluate(()=>{
      ...
      window.cameraTest={raf:window.requestAnimationFrame,cancel:window.cancelAnimationFrame,frames:new Map(),next:0,count:0};
    ...
    await open();assert.equal(await control('canvasDamping').isChecked(),false);assert.equal(await control('canvasDampingMs').inputValue(),'100');assert.equal(await control('canvasDampingMs').isDisabled(),true);
    ...
    assert.equal(await page.evaluate(()=>cameraTest.count),0);checks.push('default off / 100 ms; disabled zoom is immediate with zero damping callbacks, target or listeners');
    ...
    await open();await page.selectOption('#language','en');await open();await control('canvasDampingMs').scrollIntoViewIfNeeded();await page.screenshot({path:path.join(folder,'damping-desktop.png')});
    ...
    await open();await page.locator('#experimentsreset').click();assert.equal(await control('canvasDamping').isChecked(),false);assert.equal(await control('canvasDampingMs').inputValue(),'100');assert.equal(await page.evaluate(()=>canvasMotionEvents),null);
    checks.push('local preference survives reload, narrow layout fits, and Reset restores completely disabled / 100 ms');
