...
(async()=>{
  ...
  const assertState=async(collapse,expand)=>{for(const action of ['collapse','expand'])assert.equal(await button(action).isVisible(),true);assert.equal(await button('collapse').isEnabled(),collapse);assert.equal(await button('expand').isEnabled(),expand);};
  try{
    await page.evaluate(()=>{
    ...
    checks.push('toolbar pair keeps fixed slots with state-based disabled actions, one layout-only Undo per action, no no-op history, and unchanged group membership');
