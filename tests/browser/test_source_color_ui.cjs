...
(async()=>{
 ...
 await page.evaluate(()=>{graph.stages.pixel.nodes=graph.stages.pixel.nodes.filter(n=>n.id!=='colorReference');selected=null;selection.clear();selectedInputId=null;render();window.calls=[];window.initialGraph=JSON.stringify(graph);});
 await picker.evaluate(e=>{e.dispatchEvent(new PointerEvent('pointerdown',{bubbles:true}));e.value='#00ff00';e.dispatchEvent(new Event('input',{bubbles:true}));e.dispatchEvent(new Event('blur'));renderNativeSourceValues();e.dispatchEvent(new Event('change',{bubbles:true}));});
 assert.equal(await page.evaluate(()=>calls.length),1);assert.deepEqual(await page.evaluate(()=>nativeSourceIndex().get('tint').components.map(c=>c.value)),[0,1,0,.7]);
 await picker.evaluate(e=>{e.dispatchEvent(new PointerEvent('pointerdown',{bubbles:true}));e.dispatchEvent(new Event('blur'));e.value='#ff0000';e.dispatchEvent(new Event('input',{bubbles:true}));renderNativeSourceValues();e.dispatchEvent(new Event('change',{bubbles:true}));});
 assert.equal(await page.evaluate(()=>calls.length),2);assert.deepEqual(await page.evaluate(()=>nativeSourceIndex().get('tint').components.map(c=>c.value)),[1,0,0,.7]);
 // Canceling/reopening recaptures current TD values without submitting the canceled choice.
 await picker.evaluate(e=>{e.dispatchEvent(new PointerEvent('pointerdown',{bubbles:true}));e.value='#123456';e.dispatchEvent(new Event('input',{bubbles:true}));e.dispatchEvent(new Event('blur'));nativeSourceIndex().get('tint').components[0].value=.5;renderNativeSourceValues();e.dispatchEvent(new PointerEvent('pointerdown',{bubbles:true}));});
 assert.equal(await picker.inputValue(),'#800000');assert.equal(await page.evaluate(()=>calls.length),2);assert.equal(await page.evaluate(()=>JSON.stringify(graph)===initialGraph),true);
 checks.push('unreferenced Color survives native popup blur and polling before change; cancel/reopen reads fresh TD values without graph edits');
