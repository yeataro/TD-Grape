...
for(const [start,fraction,expected]of [[0,.2,.2],[0,1,1],[-.5,.2,-.3],[-.1,.2,.1],[12,.2,32],[-12,.2,8],[100,.1,110],[100.5,.1,200.5]]){await reset();await page.evaluate(start=>{current().nodes[0].params.value=start;render();},start);await settle();await drag(field(),await width(field())*fraction);close(await value(),expected,String(start));}checks.push('signed values and zero use the displayed decimal range; larger magnitudes scale the movement without an absolute-position jump');
for(const [start,first,second]of [[1,1.3,1.6],[9.9,12.9,15.9],[-9.9,-6.9,-3.9]]){await reset();await page.evaluate(start=>{current().nodes[0].params.value=start;render();},start);await settle();w=await width(field());p=await begin();await move(p,w*.3);close(Number(await field().inputValue()),first);await move(p,w*.6);close(Number(await field().inputValue()),second);await move(p,0);await end();assert.equal(await value(),start);assert.equal(await page.evaluate(()=>past.length),0);}checks.push('one gesture freezes its starting range across decimal boundaries, avoids acceleration, and reverses exactly without an Undo entry');
...
checks.push('Shift+Control retains a finer finite decimal increment relative to the starting range');
...
await reset();for(let i=0;i<4;i++)await drag(field('vector',i),await width(field('vector',i))*.4);assert.deepEqual(await page.evaluate(()=>current().nodes.find(n=>n.id==='vector').params.components),[.5,.6,.7,.8]);for(let i=0;i<4;i++)await drag(field('color',i),await width(field('color',i))*.4);assert.deepEqual(await page.evaluate(()=>current().nodes.find(n=>n.id==='color').params.value),[.95,.68,1.3,1.4]);await drag(field('combine').first(),await width(field('combine').first())*.4);assert.equal(await page.evaluate(()=>current().nodes.find(n=>n.id==='combine').params.components[0]),.5);checks.push('Vector XYZW, Color RGBA and scalar input defaults all support the same proportion-based gesture');
...
for(const start of[1e308,Number.MAX_VALUE]){await reset();await page.evaluate(start=>{current().nodes[0].params.value=start;render();},start);await settle();before=await snapshot();w=await width(field());p=await begin();await move(p,-w*.1);const preview=Number(await field().inputValue());assert.ok(Number.isFinite(preview)&&preview<start&&preview>start*.8,String(preview));await end();assert.ok(Number.isFinite(await value()));assert.equal(await page.evaluate(()=>past.length),1);await page.locator('#undo').click();assert.equal(await value(),start);if(start===Number.MAX_VALUE){before=await snapshot();await drag(field(),await width(field())*.2);assert.ok(Number.isFinite(await value()));assert.equal(await value(),start);assert.equal(await snapshot(),before);}}
checks.push('extreme finite magnitudes remain adjustable without an infinite range; overflow is ignored and Undo restores the exact starting number');
