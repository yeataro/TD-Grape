...
const cancellations=[['Escape',()=>page.keyboard.press('Escape')],['window blur',()=>page.evaluate(()=>window.dispatchEvent(new Event('blur')))],['pointer cancellation',()=>page.evaluate(()=>valueLadder.entry.dispatchEvent(new PointerEvent('pointercancel',{pointerId:1,bubbles:true})))],['lost pointer capture',async()=>{await page.evaluate(()=>valueLadder.entry.releasePointerCapture(1));await page.mouse.move(p.x+21,p.y);}],['disabled',()=>page.evaluate(()=>{valueLadder.entry.disabled=true;})],['Tab',()=>page.keyboard.press('Tab')],['graph replacement',()=>page.evaluate(()=>{graph=clone(graph);render();})]];
...
for(const percent of [75,125]){
  ...
  assert.equal(Number(await field().inputValue()),.3);assert.equal(await popup.locator('[data-step]:visible').count(),0);
  assert.equal(await popup.locator('.ladder-step').innerText(),'Δ 0.1');assert.deepEqual(await popup.boundingBox(),badge);
  await page.mouse.move(p.x+16,originalRows['0.01'].y);assert.equal(Number(await field().inputValue()),.5);
  ...
  await page.mouse.move(p.x,p.y);await page.mouse.down({button:'middle'});await popup.waitFor();
  const nextRows=await popup.locator('[data-step]').evaluateAll(rows=>Object.fromEntries(rows.map(row=>[row.dataset.step,row.getBoundingClientRect().y+row.getBoundingClientRect().height/2])));
  await page.mouse.move(p.x,nextRows['0.01']);assert.equal(await popup.locator('.active').getAttribute('data-step'),'0.01');assert.equal(Number(await field().inputValue()),.3);
  await page.mouse.move(p.x+16,nextRows['0.01']);assert.equal(Number(await field().inputValue()),.32);assert.equal(await popup.locator('[data-step]:visible').count(),0);
  await page.mouse.move(p.x,nextRows['0.1']);assert.equal(Number(await field().inputValue()),.3);assert.equal(await popup.locator('.ladder-step').innerText(),'Δ 0.01');assert.equal(await popup.locator('[data-step]:visible').count(),0);
  await page.mouse.move(p.x+16,nextRows['0.1']);assert.equal(Number(await field().inputValue()),.32);assert.equal(await snapshot(),before);
  await page.mouse.up({button:'middle'});await settle();assert.equal(await value(),.32);assert.equal(await page.evaluate(()=>past.length),1);assert.ok((await field().getAttribute('title')).includes('Value Ladder'));
  await page.locator('#undo').click();assert.equal(await value(),.3);await page.locator('#redo').click();assert.equal(await value(),.32);
  checks.push(`${percent}% UI / independent graph zoom: Ladder centers 0.1, keeps its compact value/step readout and increment while crossing the original list, reverses to the anchor value, and allows 0.01 only in a new gesture with one Undo/Redo commit`);
}
await page.evaluate(()=>setUIAppearance('scale',100));await settle();
await reset();const cdp=await page.context().newCDPSession(page),touch=async(type,p)=>{await cdp.send('Input.dispatchTouchEvent',{type,touchPoints:p?[{id:1,x:p.x,y:p.y}]:[]});await settle();};p=await point(field());await touch('touchStart',p);await touch('touchEnd');assert.equal(await field().evaluate(e=>e===document.activeElement),false);const oldPan=await page.evaluate(()=>clone(pan));p=await point(field());await touch('touchStart',p);await touch('touchMove',{x:p.x+15,y:p.y+40});await touch('touchEnd');assert.notDeepEqual(await page.evaluate(()=>pan),oldPan);assert.equal(await value(),.3);assert.equal(await page.evaluate(()=>past.length),0);
p=await point(field());await touch('touchStart',p);await page.locator('#valueladder').waitFor();
const touchRung=await page.locator('[data-step="0.1"]').boundingBox(),touchY=touchRung.y+touchRung.height/2,touchOther=await page.locator('[data-step="0.01"]').boundingBox(),touchOtherY=touchOther.y+touchOther.height/2;
await touch('touchMove',{x:p.x,y:touchY});await touch('touchMove',{x:p.x+16,y:touchY});assert.equal(Number(await field().inputValue()),.5);assert.equal(await value(),.3);
await touch('touchMove',{x:p.x,y:touchOtherY});assert.equal(Number(await field().inputValue()),.3);assert.equal(await page.locator('#valueladder [data-step]:visible').count(),0);assert.equal(await page.locator('#valueladder .ladder-step').innerText(),'Δ 0.1');
await touch('touchMove',{x:p.x+16,y:touchOtherY});assert.equal(Number(await field().inputValue()),.5);assert.equal(await value(),.3);await touch('touchEnd');assert.equal(await value(),.5);assert.equal(await page.evaluate(()=>past.length),1);
p=await point(field());await touch('touchStart',p);await page.locator('#valueladder').waitFor();await touch('touchCancel');assert.equal(await page.locator('#valueladder').count(),0);assert.equal(await value(),.5);checks.push('single touch tap avoids text focus, vertical swipe pans, long-press Ladder keeps its increment and compact readout across the former list, commits once, and cancels without changing the value');
