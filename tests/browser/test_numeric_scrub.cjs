...
p=await point(field());await touch('touchStart',p);await page.locator('#valueladder').waitFor();await touch('touchCancel');assert.equal(await page.locator('#valueladder').count(),0);assert.equal(await value(),.5);checks.push('touch tap focuses typing, swipe pans, long-press Ladder keeps its increment and compact readout across the former list, commits once, and cancels without changing the value');
