...
class Annotations(unittest.TestCase):
    def test_node_and_view_hash(self):
        ...
        self.assertTrue(lines[i].endswith('; // 顏色 / freely named'));self.assertIn('Comment: first line',lines[i+1]);self.assertEqual(lines[i+2],'    // second line');self.assertEqual(g,original);self.assertEqual(r['hash'],before['hash']);self.assertNotEqual(r['pixel'],before['pixel'])
        self.assertTrue(all(row['node']=='color' for row in r['sourceMap']['pixel'] if i+1<=row['line']<=i+3))
    ...
    def test_nested_and_error_mapping(self):
        ...
        start=next(row for row in r['sourceMap']['pixel'] if '// Outer call' in lines[row['line']-1]);self.assertEqual(start['node'],'outer_call');self.assertEqual(start['trail'],[]);self.assertTrue(all('_originResolved' not in row for row in r['sourceMap']['pixel']))
    ...
    def test_inline_and_dead(self):
        ...
        self.assertIn('// Inline channels',r['pixel']);self.assertIn('Comment: Inline note',r['pixel']);self.assertNotIn('Never emitted',r['pixel']);self.assertNotIn('sg_n_parts',r['pixel'])


    def test_multiline_label_and_output_statement(self):
        g=c.demo_graph('color');pixel=g['stages']['pixel']['nodes'][-1]
        pixel['params']['bufferCount']=3
        pixel['ui'].update(label='Outputs\n#error inert',bufferLabels={'buffer2':'Auxiliary'})
        r=c.compile_graph(g);lines=r['pixel'].splitlines()
        self.assertTrue(next(line for line in lines if 'for (int sg_buffer' in line).endswith('{ // Outputs'))
        self.assertEqual(next(line for line in lines if '#error' in line),'    // #error inert')
        self.assertNotIn('Auxiliary',r['pixel'])
        self.assertTrue(all(row['node']=='pixel' for row in r['sourceMap']['pixel'] if 'Outputs' in lines[row['line']-1] or '#error' in lines[row['line']-1]))
    ...
    def test_top_sampler_comment_keeps_text(self):
        ...
        self.assertIn('// sg_sampler_'+decl['id'],r['pixel']);self.assertIn('texture(sTD2DInputs[0]',r['pixel'])
