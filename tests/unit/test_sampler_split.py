...
class SamplerSplit(unittest.TestCase):
 ...
 def test_legacy_definition_identity_preserved(self):
  ...
  self.assertEqual(c.digest({k:v for k,v in c.CATALOG.items() if k not in ('sampler','texture_sample','constant','top_input','glsl_code','vec4','combine','vector_split','swizzle','vector')}),baseline['catalogHash'])
