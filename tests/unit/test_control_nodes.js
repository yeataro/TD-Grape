...
const branchTypes=valueTypes();
identical(selectableNodeTypes(d('compare')),['float','int','uint']);identical(selectableNodeTypes(d('if')),branchTypes);
...
for(const ty of branchTypes){const branch=node('if','branch',{type:ty});assert.equal(defaultInput(branch,'condition','bool'),false);identical(defaultInput(branch,'true',ty),filledValue(ty,1));identical(defaultInput(branch,'false',ty),filledValue(ty,0));}
