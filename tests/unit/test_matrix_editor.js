...
const conversion=catalog.find(d=>d.key==='convert'),matrixConversion=catalog.find(d=>d.key==='matrix_convert');
assert.ok(creatorVariants(conversion,{kind:'outputs',type:'mat2'}).some(v=>v.outputs.out==='vec4'));
assert.ok(creatorVariants(matrixConversion,{kind:'inputs',type:'mat2'}).some(v=>v.inputs.value==='vec4'));
same(creatorVariants(conversion,{kind:'inputs',type:'mat2'}),[]);
assert.equal(convertSources('matrix_convert').length,28);assert.equal(convertSources('matrix_convert').includes('vec3'),false);
...
const invalidOutput=clone(typeContract);invalidOutput.convert.outputTypesByNode.convert.push('mat3');assert.throws(()=>setTypeContract(invalidOutput));
