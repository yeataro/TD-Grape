...
class TypeContract(unittest.TestCase):
 def test_contract_is_pure_and_deterministic(self):
  ...
  self.assertEqual(c.digest({k:v for k,v in c.CATALOG.items() if k not in (*c.MATRIX_KEYS,'sampler','texture_sample','constant','top_input','glsl_code','vec4','combine','vector_split','swizzle','vector','replace','spec_constant','comment','compare','if','sign','sqrt','floor','round','ceil','trunc','mod','rgb_to_hsv','hsv_to_rgb','remap','range_from','range_to','loop','zigzag','perlin_noise','simplex_noise','scalar','convert')}),baseline['catalogHash']);self.assertEqual(c.digest(legacy_function_library()),baseline['libraryHash'])
 ...
 def test_existing_graphs_produce_identical_results(self):
  ...
  for key,d in c.CATALOG.items():
   if key in (*c.MATRIX_KEYS,'sampler','texture_sample','constant','top_input','glsl_code','vec4','combine','vector_split','swizzle','vector','replace','spec_constant','comment','compare','if','sign','sqrt','floor','round','ceil','trunc','mod','rgb_to_hsv','hsv_to_rgb','remap','range_from','range_to','loop','zigzag','perlin_noise','simplex_noise','scalar','convert'):continue  # New nodes have dedicated tests; keep all 138 old fingerprints.
