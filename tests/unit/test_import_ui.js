...
const context=vm.createContext({assert,console,crypto:globalThis.crypto,location:{pathname:'/',hash:''},history:{replaceState(){}},
  document:{querySelector(s){if(!elements.has(s))elements.set(s,element());return elements.get(s);},createElement:element},
...
for(const name of ['functions_model.js','functions_ui.js','graph_ui.js','import_ui.js'])vm.runInContext(fs.readFileSync(path.join(dir,name),'utf8'),context);
...
assert.equal(JSON.stringify(graph),JSON.stringify(document));assert.equal(JSON.stringify(past.at(-1)),JSON.stringify(initial));assert.equal(future.length,0);
undo();assert.equal(JSON.stringify(graph),JSON.stringify(initial),'one Undo restores pre-import graph');
...
await reviewImportFile({name:'good.json',size:50,text:async()=>JSON.stringify(document)});
assert.equal(acceptImportReview(),true);assert.equal(graph.functions.length,0);assert.equal(past.at(-1).functions.length,64);
