...
Graph 的待套用新增可整筆復原。原生來源一旦已由 TD 接受，Graph Undo 只移除引用，保留來源、型別與原 ID；避免輪詢把同一個原生列重新辨識成新來源。實際來源刪除走 Parameter 的獨立操作，不能把 Graph Undo 當作 TD 來源刪除。 刪除成功後同步完整來源清單，清除過期的圖 Undo／Redo 與原生來源快取，避免舊引用操作讓已刪除的來源復活。來源變更待套用時，僅本地圖與同 revision 的來源快照完全相同才能繼續恢復／刪除操作；有另外的未提交編輯時仍保護草稿，輪詢不覆蓋它。
