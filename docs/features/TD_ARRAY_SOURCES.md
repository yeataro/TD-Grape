...
- A native CHOP array uses a positive literal or a reference to an integer Graph Constant. Since 0.8.123, external array types are no longer restricted by the graph constructor's 1024-element authoring limit. Literal type lengths use the positive signed 32-bit range; actual resource capacity remains a host/compiler limit, not a promise that every length will compile. Imported sources keep `value: null`, and large references do not expand into UI defaults. The source inspector can switch between literal and constant-reference forms. Graph Constant references retain their identity and generated GLSL name; checking the CHOP sample count reads the already stored literal, without evaluating an expression chain or querying a live specialization value.
