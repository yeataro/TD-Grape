...
2026-09-19 更新：使用者已授權先實作 Uniform 即時通訊。第一批 scalar／vector 數值的實作、界線與驗證見 [Uniform 即時數值](../features/UNIFORM_LIVE_VALUES.md)。下文保留原始調查與候選方案，不代表本批全部採用。

2026-09-18。狀態：原始碼調查、隔離瀏覽器重現與官方機制核對；尚未修改產品傳輸或來源控制方式。這是獨立於 Matrix 的調查評估，不將候選方案當成已批准的實作。

## 後續介面修訂觀察（2026-09-19，僅記錄）

- 使用者觀察 `uAbsTime` 持續讀值時，X 數字欄與下方固定的來源控制說明都被 DevTools 標示為重繪。數值更新本身正常；疑問是說明文字為何跟著重繪。
- 使用者也回報「數值來源 / Python Expression」區域目前沒有呈現預期資訊。截圖中的該區為收合狀態，尚未核對展開內容、來源模式與顯示條件，不先判定為資料缺失。
- 綠色重繪標記只能證明區域被 paint；尚未確認說明內容有變、DOM 有重建，或只是連帶重繪，也未證實與既有全域 busy／停用樣式閃動同源。
- 依使用者要求暫不調查或修改；待 Uniform／Sources 介面與能力修訂時一併檢視來源資訊的呈現，以及持續值更新應影響的 UI 範圍。
...
## 2026-09-20 多來源／圖內引用檢查點（0.8.125）

圖內 scalar／vector Uniform 引用沿用 Parameter 的分量控制。IntersectionObserver 管理實際可見的控制，同一來源的多個引用及 Parameter 去重成同一訂閱；隱藏或畫布外的控制退訂，隱藏分頁斷線。一次批次訂閱只解析一份圖並建立 ID 索引，不逐來源重讀圖。值推送在瀏覽器同一幀合併，只更新變動來源的控制，不存圖、不重建清單。

原生隔離測試含 200 個不同 float 來源、重複 ID、缺失 ID 及手勢／Bind／Undo／外部改值競爭。首次測得全 200 項每輪讀值約 18.75 ms；改成只讀宣告分量後約 4.86 ms。訂閱初始化原先約 40.22 ms，追蹤發現反覆查詢 TD 不存在的可選 Type 參數開銷大；一次查詢可用 Type 欄位後約 14.21 ms。這些是本機單次壓力測試，不是其他硬體或任意來源的效能保證。使用者接受極端規模較少見，不要求為此大改架構；仍需區分初始化一次成本與每輪讀值成本。來源讀值 tick 維持至多 30 Hz，只評估訂閱的分量。

13 組原生檢查及 10 組瀏覽器檢查通過；200 項一次 tick 每個 ID 只求值一次，只送兩個實際變動來源，沒有原生 inventory scan、圖讀寫或編譯。100 次手勢更新仍只在 commit 記一筆 Undo。缺失或型別變動會使相應訂閱失效，保留既有 CAS／原生 Parameter 身分檢查，未解除任何 Bind。Common／Custom 卡片會使用同一套訂閱及控制，接續實作。
