...
因此「不使用幾何點色」与「不使用實例色」不是同一個開關。畫面不對時，需核對是哪一層顏色被省略，不能把所有顏色乘法當成一個可任意刪除的步驟。
...
| 有效陰影、Fog、POP 三平面與多輸出场景 | 編譯通過但實際分支未生效的假驗證 |
...
以下雜湊為本次紀錄 SHA-256 的前 12 碼，用於辨識參考內容，不是新版本 TD 的固定輸出要求。`enabled` 指本次配置的相容功能集合，不代表所有互斥功能同時存在。
