...
## Uniform 初始化與 footer 置中（2026-09-20，0.8.143 實作）
...
- Uniform 與 Scalar 數值滑桿的外觀：使用者確認保留 Uniform 的 32px 最小高度，允許與 Scalar 的 24px 採不同樣式。Uniform 滑桿需更醒目，已提供三個獨立滑桿示意供比較：A 原填色加型別色細線、B 型別色填滿、C 淡型別填色加細線（建議）。把手放在填色分界，兩端內縮以避免 0／滿格時消失；沿用現有十進位範圍顯示，不把所有十的倍數都視為滿格。使用者尚未選定外觀，產品樣式未修改。
