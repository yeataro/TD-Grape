...
- Uniform 與 Scalar 數值滑桿的外觀：使用者確認保留 Uniform 的 32px 最小高度，允許與 Scalar 的 24px 採不同樣式。Uniform 滑桿需更醒目，已提供原樣加三個候選示意：A 原填色加 Uniform 亮色細線、B Uniform 青綠分類色填滿、C 淡 Uniform 填色加細線（建議）。使用者要求忠實沿用目前 Uniform 的背景、字色、字級、尺寸及辨識色；已撤掉最初以 float／vec 輸出口色代替的示意。把手放在填色分界，兩端內縮以避免 0／滿格時消失；沿用現有十進位範圍顯示，不把所有十的倍數都視為滿格。使用者尚未選定外觀，產品滑桿樣式未修改。
