...
以 0.8.142 已交付內容為基準。本次只整理後續順序；下表不是完成紀錄。使用者已確認 Spec Constant 新建預設名稱採 specValue，加入修改清單；產品預設仍待實作。其他仍在思考的操作／位置不因列入此計畫而視為定案。

| 順序 | 工作 | 範圍與完成條件 |
| --- | --- | --- |
| 1 | 修正圖外觀變更暫鎖 Uniform | 分辨圖能力／內容、來源定義／原生值、位置／外觀的更新與保護範圍；驗證保存、即時手勢、切換 Shader 及 Undo／Redo，不直接移除整體保護。 |
| 2 | 預設命名與 UI 小修 | Spec Constant 新建使用 specValue 並沿用既有唯一命名規則，既有來源不批次改名；Appearance 設定列左右等寬、對齊 Dark／Light 中線，主題與介面大小面板的上方設定列下加水平分隔線。Language 置中方式待目視確認。左下角 ⋮ 的留白已隨 0.8.143 footer 錯誤修正。 |
...
- Spec Constant 前綴已定案：使用者接受新建預設採 specValue（spec 對應 Spec／Specialization Constant），與 sampler 的 s 前綴區分，取代先前「先保留 sValue」的判斷。加入後續修改順序，沿用既有唯一命名規則；產品預設尚未修改，既有來源不批次更名。
...
- 後續排版回饋（待修改）：Appearance 標題與下拉控制各占一半可用寬度，以 Dark／Light 等寬選項的中線為對齊基準；方向為左右各自置中，下拉箭頭仍在控制右側。兩個面板均在上方設定列與下方選項之間增加水平分隔線：Appearance 與 Dark／Light 之間、Language 與 Standard／Comfortable 之間。線條跟隨面板內容寬度，Appearance 在 Light 隱藏時不留下孤立分隔線。Language 是否也採等寬置中仍未定案；建議沿用相同排版以維持一致，實作時先比較長語言名稱及窄面板效果，不當成使用者已確認。
