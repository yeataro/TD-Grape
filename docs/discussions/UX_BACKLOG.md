...
## Color Uniform 自動分量命名（2026-09-20，0.8.149 已修正）

原本自動 Split 與接線建立選單只辨識 Color RGBA 節點，漏掉 TD Colors 來源。現在依持久化的 Uniform `nativeSequence=color` 給新建向量操作 RGBA 名稱，不依賴來源快照到達、節點名字或目前色值；一般向量及 UV 維持原命名。既有 Split 的命名可由使用者手動設定，因此不自動覆寫，原有 XYZW 可在 Settings → 分量名稱改為 RGBA。實際 port ID／接線不變，建立與手動命名仍可 Undo／Redo。兩個建立入口、既有選擇保留、手動改回 XYZW、重載與一般向量區別均驗證。
