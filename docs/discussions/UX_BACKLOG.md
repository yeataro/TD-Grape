...
## Uniform 填色亮度後續比較（2026-09-20，待選擇）

使用者認為 0.8.144 的 Uniform 填色仍不夠顯眼，要求明度達到原 Scalar 滑桿或更高、降低彩度，並比較柔和一點的把手。新的方案預覽需包含「原 Scalar 滑桿底色＋把手」，並畫出完整 Uniform 節點。候選配色先以 Visualizer 比較，尚未改動產品配色；介面亮度不影響滑桿的要求保留。
