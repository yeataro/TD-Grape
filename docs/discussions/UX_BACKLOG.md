...
## 2026-09-23 節點程式碼預覽與 GLSL 行號對應（重構後的未來功能，僅記錄）

- 暫稱「節點程式碼預覽／算式預覽」：顯示該節點本身產生的程式碼，依目前選定的型別、輸入型別及運算組合，整理成簡短的一行程式碼／算式。
- 使用接孔名稱或 Input1、Vector 等可讀識別，呈現實際型別轉換及運算，例如 vec3 先轉成 vec4，再與另一個 vec4 相加；不直接顯示難以閱讀的內部 UUID。
- 未來可提供顯示開關，將算式放在節點上方，以透明背景浮動顯示。
- 節點內部是否需要建模成一張圖尚未決定；本需求不以此為必要條件，也不等同於顯示整份 Generated GLSL。
- 另記 GLSL 行號與節點的雙向互動：在 GLSL 顯示器選取程式碼行時，標記其對應節點；反過來選取節點時，高亮 GLSL 中對應的行號或整段程式碼。行號到節點的可靠對應、一行多節點或單節點多行等情況留待設計；本次不宣稱現有對應資訊已足夠。
- 使用者明確指定為「重構後的未來功能」，目前無法快速完成，這次不實作。格式化來源、長算式截斷及介面細節留待重構設計。

## 2026-09-23 Math 連續四則運算節點（0.8.205 已交付）

- 使用者最新確認名稱為 **Math**（語音原為 Max），將連加、連乘、連減、連除合併為一個節點，以參數頁下拉選擇；既有二元四則與取最大值 Max 保留。
- 先依使用者要求只做對話內草圖；後續已確認草圖並授權依簡單算式註記方式處理。現行 [Math](../features/MATH.md) 提供「全部輸入共用運算」及「逐列選擇運算與輸入」，預設運算皆為加法。
- 草圖候選：輸入 A…Z、AA、AB…；逐列使用上一列結果作左側。連減／連除按列由前至後計算，不採一般算式的乘除優先。這些排列已依草圖落地，Output 保持在輸入下方，不放在最上方。
- 接孔操作構想：參考 Input／Output 的動態接孔，尾端保留灰色待新增接孔，接線後自動增加輸入；Math 已實作此操作。
- 這種接口暫稱 **待新增接孔（Spare Port）**：呈灰色的預留新增入口，尚不是正式輸入／輸出，接線時才建立正式接孔。這是暫定用語，本輪不更改既有 UI 標籤。
- Math 歸入數學分類；型別與實作邊界見功能文件。臨時算式只是運算元的關係文字，直接寫入既有註記，不是程式碼預覽，也不包含重構後的行號對應能力。

## 2026-09-23 此 Shader 的 Subgraph 定義保留（已知問題，僅記錄）

- 刪除最後一個畫布上的 Subgraph 引用，不會刪除 Shader 內保存的定義。現有資源清單依定義及目前 Stage 篩選，因此未使用的定義仍可能列出；「此 Shader」不能解讀為只列圖上正在使用的節點。
- 使用者要求先列為資源管理的已知問題。目前沒有新增刪除功能的需求，也不實作未使用定義的自動清理；保留現有資料與行為。
- 個人調色盤維持由使用者到來源資料夾移走或刪除檔案，再重新讀取，不新增介面內的刪除入口。此行為與 Shader 內定義的保留分開看待。
- 已討論的資源庫整併方向為「全部／此 Shader／內建庫／個人」（All / This Shader / Built-in / Personal），取代獨立的此專案區塊；此處僅保存決策背景，分頁整併已於 0.8.203 實作。Shader 指包含 Vertex、Pixel 與共用定義的一份 Shader，不是整個 TD Project；歸屬同一 Shader 不代表能跨 Stage 使用。

## 2026-09-23 資源庫重複建立入口（已隱藏，0.8.203）

- 資源庫裡的「建立 Subgraph」與「從選取建立 Subgraph」先隱藏。使用者認為圖內操作、框選後的編輯功能與快捷鍵已足夠；不移除底層能力，不新增替代入口。後續如需調整可再討論。

## 2026-09-23 Swizzle 操作一致性（已知問題，待處理）

- 使用者回報：Swizzle 目前依靠參數面板的設定增減分量，與目前整體節點操作規範不一致。此處指 **Swizzle**，不是 Switch。
- 記為操作一致性的已知問題；後續需評估如何與既有節點的分量／接孔操作方式統一。這次只記錄，尚未選定替代互動方式，也不修改節點行為。
- 使用者目前未見其他節點採相同的面板操作方式；是否還有其他例外尚未全面查核，不將「Swizzle 是唯一例外」當作已確認結論。

## 2026-09-23 個人資源庫筆記（待處理）

- **已知問題：Subgraph 資源目前沒有分類編輯入口。** 個人庫與此 Shader 的分類管理先記錄，不在 0.8.203 分頁整併中新增功能；未來若處理，再評估分類與來源 metadata 的保存方式。
- 增加把 Subgraph 存入個人庫的明確入口，不只依賴拖放。
- **已解決於 0.8.201：Subgraph 的簽名與說明容易被理解為 GLSL 函式呼叫。** 實際 `sgrape_core._expand()` 會在各使用位置遞迴展開子圖，再交由 `_compile_flat()` 產碼；不會因 Subgraph 邊界自動產生 `Tint(...)` 函式。共用定義、雙擊進入及 Make Independent 的編輯語意仍有效，已統一新建名稱、邊界、選單與說明為 Subgraph，資源庫改成輸入／輸出列表；既有使用者命名保留。此項是說明問題，不是已確認的渲染錯誤。

前兩項仍保留待辦；Subgraph 命名與說明依使用者後續要求優先修正。

2026-09-21：跨輪次的最新分類見[待辦總整理](TODO_AUDIT_2026-09-21.md)，已區分未完成、候選、已完成及被取代的歷史決定。

## 0.8.162 導覽 review 修正

一階 Ctrl＋左右保留原選取、增選直接上游／下游，可從多選反覆逐階擴展；未勾選仍一次全選方向上的所有節點。方向鍵以依畫面位置為預設，其他兩版暫留。原自動 Frame 勾選改為四個視角選項：不移動／Frame 有動畫／Center 有動畫／Center 無動畫；Center 獨立處理平移，完全保留倍率。此段覆蓋下方 0.8.161 的一階替換選取、雙向分支預設與自動 Frame 開關描述。

這裡記錄機制本身合理、但使用體驗仍值得評估的情境；不代表已確認為程式錯誤，也不代表已接受為永久限制。

## 0.8.161 方向導覽比較與接線選取（2026-09-21，已實作）

- 實驗功能的「方向鍵導覽方式（試用）」保留原版路徑，新增預設的雙向分支及依畫面位置版本。雙向分支把最近左右移動的參照和返回歷史分開，處理 A → B → 回 A 後仍可切換 B 其他上游的情況。位置版依節點中心與方向搜尋，優先前方扇區，再比較距離，可從滑鼠單選直接上下移動。三版本都單選限定、沒有循環跳轉。
- 最終快捷鍵以使用者更正為準：Ctrl＋左／右選所有上游／下游且保留起點，Ctrl＋上選整個相連區塊，Ctrl＋下選當前圖中全部不相連節點。多選以所有起點的聯集計算。L／Shift＋L 的自動排列不改；沒有採用 Alt＋L 或 Ctrl＋L。
- 獨立實驗選項「Ctrl＋左右只選下一階」預設關閉；開啟後改選直接相連的全部鄰居，終點停住。Ctrl＋上下仍處理完整相連區塊／補集。按住不連發，避免補集反覆切換。文字欄位、選單、編輯手勢及待放置節點時不介入。
- Ctrl＋Enter 在畫布切換專注編輯，反覆按可進出；Esc 取消退出專注編輯的行為，只保留取消／關閉。文字欄位的 Ctrl／Cmd＋Enter 提交仍保留。Alt＋Enter 全螢幕不變。
- 快捷鍵按作業系統自動顯示 Ctrl／Cmd，移除 Help 的 Mac 例外段落；新增命令同時接受 Control／Meta。共用平台標籤也用於實驗選項、剪貼簿及文字提交提示；原有只接受 Ctrl 的數值拖曳提示未改成 Cmd。Mac／Safari 實機留待使用者驗證，本輪只有 Windows 與模擬平台資訊測試。
- 詳細規則記於 `docs/ui/UI_NAVIGATION.md`。設定可直接切回原版；已知右鍵框選放開於 Slider 的選單問題及 Group 自動排列仍維持後續筆記。

## 0.8.159 連線層級簡化（2026-09-21）

依使用者最後指定的層級：節點 → 所有可見線 → 所有透明命中線 → Group 本體。其他元件維持既有前後關係。取代 0.8.157–158 每條線各自交錯的 hit／paint 排法，現在先 append 所有透明路徑，再 append 所有可見路徑；各組內仍保留原連線順序。移除每條線的額外 SVG group，命中路徑直接引用實際路徑，沒有新增 Group 專用命中判斷。新增相鄰兩條線的可見描邊優先於另一條透明延伸區的 hover／點擊／右鍵檢查；命中檢查共 9 組通過。

## 0.8.158 方向鍵與本輪回饋（2026-09-21，已實作）

- 單選節點的左右接線導覽、原路返回，以及上下切換上一段分支。新滑鼠選取後先以左右開始；多選／無選取／單選線不進入此節點導覽，端點不循環。每次按鍵搜尋，重複連線只停一次，同高以 X／ID 穩定決勝；失效的路徑清除。自動 Frame 為瀏覽器偏好，預設關閉；快捷鍵 Help 已加入說明。
- 使用者實測 0.8.157 後要求加粗命中區：改為最小 6 CSS px，放大後隨比例增大；可見線不變。重疊測試發現 Group 控制可能被透明線攔截，已讓 Group title／corner 位於線之上，節點仍優先，背景層次保留。
- 使用者補充命中優先權以視覺可見為準：Group 本體不遮擋可見連線，也不阻止其 hover；只有實際蓋住線的節點、標題及把手等控制項優先。現有分層符合此方向，另補 Group 框內仍可 hover 的實際檢查。
- 臨時節點放置的最新定案：使用者撤回恢復紫色的要求，保留選取綠色，加 32% 黑色底，可透出後方畫布與線。Excellent／Legendary／Godlike 加 6px backdrop blur；Simple／Professional／Cool 保持單純半透明黑底。原紫色深色 #b39cfb、淺色 #7452aa 仍記錄供後續選擇；沒有提高亮度。
- 右鍵框選在 Slider 放開誤開預設值選單仍是待修筆記；Group 自動排列仍待評估。本輪未混入這兩項。

驗證：新方向鍵 11、命中區 7（含 15 組圖／UI 縮放）、放置 10、設定 20、Group 13、選取／工具列與快捷鍵 Help、數值滑桿 32 組回歸及完整 portable checks 通過。毛玻璃截圖已檢視；報告 reports/arrow-navigation-158/。

## 右鍵框選於 Slider 放開誤開選單（2026-09-21，待修正）

使用者回報：右鍵拖曳框選多個節點後，在 Slider 上放開會彈出預設值選單。預期只完成框選，不應把同一手勢的結束當成 Slider 右鍵操作；一般在 Slider 上右鍵仍應正常開啟預設值選單。先記錄，未混入已授權的連線命中寬度與方向鍵導覽修改。

補充建議：使用者提出改成在 Slider 上按下右鍵時即開選單。後續檢查手勢起點及瀏覽器放開後的 contextmenu，避免單純提前開啟卻仍被放開事件重複觸發；保留鍵盤開選單等非滑鼠入口。尚未實作。

## 0.8.157 連線命中區與放置預覽（2026-09-21）

已實作每條連線原層級內的透明命中路徑，螢幕最小寬度 2.5 CSS px，放大後跟隨可見線；不更改可見線寬。路徑成對保留順序，僅可見路徑持有接線語義，滑鼠、觸控及右鍵共用實際邊。使用者追加要求：放置預覽外框／文字／Glow 改用選取色（深色 #70dc69，淺色 #357a4d），臨時線仍為接孔原色。舊預覽紫色為深色 #b39cfb、淺色 #7452aa，可由兩處 CSS 恢復；Glow 強度、Simple 規則及亮暗度獨立性保留。

本輪已授權開始：先驗證、提交與交付命中區，再不中斷接著做方向鍵導覽。方向鍵方案以最後 review 通過的版本為準：左右沿接線／按已走路徑返回；上下切換上一段的分支；新滑鼠選取重設路徑，先左右建立方向；多選不介入、不循環、按鍵才搜尋。下方需求收集時的「尚未授權開始」為歷史狀態。

## 連線最小命中寬度原始評估（2026-09-21，0.8.157–158 已實作）

使用者已批准加入下一輪，與方向鍵導覽一起累積需求；目前仍在補充筆記，等補充完成後再一起動工。

可見線條繼續隨圖縮放；命中區以螢幕 CSS px 保留最小寬度，放大後隨線條變寬。目前普通線寬 2.5px，可作初版下限。隔離 Chromium 原型驗證 12.5%–200% 圖縮放與 75%–125% 介面縮放，透明路徑能接收點擊及右鍵。500 條線的建立與樣式／排版處理中位數由 1.6ms 至 2.9ms，這不是完整產品 FPS 或 GPU 效能結論。報告位於 Documents 工作區 reports/wire-hit-study/。

使用者強調「不要帶來任何操作副作用」，尤其透明線的堆疊順序。後續實作的必要驗收條件：

- 命中路徑留在原本連線層，與對應連線維持相同順序；不能另外把整批透明線提升到節點、接孔、Group 控制項或工具列之上。
- 交叉／重疊線維持原本前後順序的選取優先權；hover、點擊、右鍵及拖曳均指向同一條實際連線，不能有透明路徑殘留或索引錯配。
- 命中區以外的空白畫布拖曳、框選、縮放，以及節點／接孔／Group／選取工具列的既有操作維持原樣。節點覆蓋線條時，線不能攔截上層節點操作。
- 透明路徑不繼承可見線的 Glow、hover 顏色或選取描邊，也不參與可見圖形邊界計算；單次操作不能因兩條 DOM 路徑重複觸發。
- 以上需在縮小、放大、介面縮放、交疊與拖曳情境驗證，再決定是否交付；保留既有圖資料、Undo 及 TD 同步語義。

## 自動排列與 Group 邊界（2026-09-21，下一輪評估筆記）

使用者觀察自動排列一開始未納入 Group，並擔心把整個 Group 當作一顆大節點會拉大左右間距；目前尚未決定修改方式，也可能維持現況。

程式核對：`arrangeSelection()` 只將選取節點的實際尺寸及連線交給 `autoArrangePositions()`；Group 邊界則由成員排列後的位置加上 padding／標題列重新計算，沒有參與排列約束。現有演算法以每層最大節點寬度決定下一層位置，因此直接把大型 Group 放進同一套層級排列，確實可能把整層間距一起撐開。

評估方向而非定案：區分「排列 Group 內部節點」與「保留內部配置、排列整個 Group」，再評估局部邊界避讓；不能只替換成 Group 外框尺寸便視為完成。需考慮跨 Group 接線、部分成員選取及是否改動未選取節點。先保留現行行為，不藉本筆記變更全域排列或 Group 規則。

## 節點與接線操作審查（2026-09-21）

本輪批准範圍、放置預覽定案及 TD 插入調查見 [節點與接線操作審查](NODE_PLACEMENT_REVIEW.md)。插入節點及明亮模式全面檢查保留後續。

## 0.8.155 Uniform 邊界第一版（2026-09-20）

使用者批准「照方案先試一個版本」。已實作 [方案 B 第一階段](UNIFORM_SYNC_BOUNDARY_STUDY.md)：未改變的數值 Uniform／Color 不因一般圖修改或重新編譯而停用，來源本身、原生目標及 expected 值仍核對。修正舊 HTTP／訂閱回覆覆蓋新值，以及面板重畫短暫退訂；保留圖保存衝突與草稿基準。第二階段的原生來源列表變動局部恢復，以及跳過相同 GLSL 的重編譯未實作。下面 0.8.153–154 段落保留 review 歷程，其「待討論／尚未實作」描述的是當時狀態；本版完成範圍與限制以上段及研究文件第 7 節為準。

## 0.8.153 後 review 回饋（2026-09-20，UI 小項已於 0.8.154 實作）

本輪使用者已先結束 review，要求統整「何時觸發」與「何時真正需要保護」，並進一步指出圖有差異本身不是停用 Uniform 的理由。程式核對、隔離產碼比較與建議原則見 [Uniform 可編輯狀態與圖更新的分工](UNIFORM_EDIT_PROTECTION_REVIEW.md)；新保護規則尚未實作，等待架構討論。

後續依使用者「研究方案」完成 [Uniform 同步邊界研究與方案比較](UNIFORM_SYNC_BOUNDARY_STUDY.md)：隔離瀏覽器核對 10 種編輯、10 種旗標及讀數順序，TD 9 項情境驗證實際 Par／手勢。建議局部整理來源判斷及讀數接收，先保留圖格式與完整提交；仍待審查，使用者的架構釐清不是新方案定案，本輪沒有修改產品或部署。

最新工作優先順序：使用者明確改為先完成 UI 小項，再討論架構與 Uniform 保護。0.8.154 僅處理下列滑桿、Vertex 按鈕及 Note 呈現；圖傳輸、保存、編譯和保護判斷均未修改。

- 0.8.154：Vertex Stage 按鈕採用 TD Position 所屬 attribute 系列的既有配色，深淺色均對應；範圍限 Stage 按鈕。

功能操作目前先記錄使用者觀察到的保護觸發情境；使用者明確要求待回報收齊後，再一起判別哪些確實需要保護、哪些不應讓 Uniform 灰掉，不把下列所有現象直接定性為錯誤或要求解除保護。

- 0.8.154：移除 Uniform slider 在填色分界處的線條把手；此決定取代先前「淡填色＋把手」。保留灰青提亮配色及不隨介面亮暗度變動的規則。
- 0.8.154：畫布 Uniform slider 與一般滑桿高度一致（滑鼠 24px、觸控 30px）；Parameter／Inputs 沿用相應的一般分量高度。初始化佔位與模式文字使用相同畫布列高，避免非同步資料到達時跳動；分量標籤跟隨欄位中心。
- 功能 review：使用者實測移動節點位置，已不再觸發 Uniform 突然灰掉；但刪除不在運算中的節點仍會觸發，列為待判別的保護觸發情境，與上方滑桿外觀要求分開追蹤。使用者明確定義：從 Color Output 沿輸入連線往回追溯，不在該上游節點流程中的節點；即使這些節點彼此有連線，只要不在 Color Output 的上游流程中，也屬此範圍。後續依此範圍重現，檢查刪除是否仍被整體圖內容比較視為需要保護 Uniform 的變更，並核對實際來源身分與依賴影響。使用者持續 review 中，本輪先累積此回報，尚未修改刪除判斷。

- 功能 review 補充：更改節點自訂名稱也會讓 Uniform 暫時灰掉。與刪除不在 Color Output 上游運算流程中的節點一起追蹤；目前先記錄，待本輪回饋收齊後處理。
- 功能 review 補充：修改常數的數值也會讓 Uniform 暫時灰掉。先記錄此現象，保護是否必要待與其他操作一起判別；目前尚未修改常數編輯或保護邏輯。
- 功能 review 補充：修改不在 Color Output 上游運算流程中的 Combine 節點型別，也會讓 Uniform 暫時灰掉。「跟圖無關」沿用上方從 Color Output 往回追溯的定義；先記錄，保護是否必要待一起判別，目前尚未修改型別編輯或保護邏輯。
- 功能 review 補充：使用者觀察修改運算流程中節點的各種設定，都可能觸發 Uniform 暫時灰掉，包含修改分量名稱。此處的運算流程同樣指 Color Output 的上游節點流程；保留「可能觸發」的觀察，不推定每個設定都已逐一驗證，也不預先認定其保護必要性，待本輪回饋收齊後分類判別。
- 功能 review 補充：使用者進一步確認，即使節點不在 Color Output 上游運算流程中，修改其分量名稱也會觸發 Uniform 暫時灰掉。此項補充流程外的情況，不取代前述流程內的觀察；目前先記錄，保護是否必要待一起判別。

## Note 標題與選取外觀補充（2026-09-20，0.8.154 已實作）

使用者將此項列為與 Uniform 保護議題無關的次要筆記，不希望為 Note 再增加許多例外或更改全局規則：

- 標題列右側仍顯示一個小字 Note，使用者質疑其必要性。程式核對：它來自一般節點在啟用自訂名稱且有 `n.name` 時顯示原型名稱的共用 subtitle；並非 Note 內容。
- 標題列出現時，body 上方兩個圓角應消失，讓標題列與 body 接成完整外觀。最上方仍由標題列的圓角形成輪廓。
- 選取時整個可見 Note 都應呈現一致的選取亮示，包含出現的標題列；目前綠色外框只包 body。
- 0.8.154 已沿用 Note 既有標題顯隱狀態，移除小字原型名稱、接合標題與 body 的圓角，並以不佔尺寸的輪廓包住整個可見選取。修改局限於 Note 呈現層，不改 body 座標、尺寸、Group 邊界、命中範圍或全局節點規則；未加入 hover 顯示標題。

## UV 初始化阻斷（2026-09-20，0.8.152 緊急修正）

0.8.150 的分量名稱回歸漏掉固定 UV 節點沒有 params.type 的情況，繪製接孔時把 undefined 當型別解析，誤顯示「無法讀取型別規則」，不是重新開啟可以恢復的問題。現在依固定輸出口的 vec2 描述取得分量數；未停用型別規則驗證。新增真正初始化與重開回歸，並以四張使用中圖的副本驗證完整載入。此錯誤優先於未完成的 Uniform 保存保護調整。
...
| 順序 | 工作 | 範圍與完成條件 |
| --- | --- | --- |
| 1 | 修正圖外觀變更暫鎖 Uniform（0.8.153 已實作） | 位置／尺寸草稿及確認沒有重建 Shader 的保存不再停用 Uniform 值；即時通道保留原生身分，REST 等待保存回覆並核對新版本。來源設定與圖內容變更仍保護。 |
...
## 圖變更範圍與來源可編輯狀態（2026-09-20，0.8.153 已修正此批範圍）
...
- 0.8.148 後使用者再次觀察調整節點大小及位置都仍會讓 Uniform 暫時灰掉／鎖住。0.8.153 將 Uniform 數值／Color 控制與來源設定的可編輯條件分開，沿用既有圖內容比較，位置／尺寸草稿與保存中可繼續調值；這與已修正的初始化尺寸／Group 邊界問題不同。
- 即時手勢沿用原生 Parameter 身分檢查。REST 值寫入等待在途 layout 保存，再確認載入世代、內容與來源版本；只有 TD 明確回覆未重建 Shader，且原快照版本及內容吻合，才推進該快照版本。舊輪詢不得倒退版本；真的重建時等待新快照。後端 CAS、Binding 目標核對與 Undo 規則不放寬。
- 已驗證改名／改型別／刪除／接線草稿、保存衝突、保存途中新增內容修改、切換載入、唯讀及交錯 Undo／Redo。來源設定、Spec Constant、Matrix／Array 設定保持原有保護；此批不宣称完成所有圖能力的分類重構。
...
## 方向鍵導覽與 Frame（2026-09-20，決策過程；0.8.158 已實作）

以下保留討論過程；最終範圍以上方 0.8.158 及 docs/ui/UI_NAVIGATION.md 為準，舊方案或「尚未動工」敘述不代表目前狀態。

2026-09-21：使用者要求加入下一輪，與連線最小命中寬度一起處理；先累積補充，尚未動工。最新明確澄清「線不用選」：方向鍵直接選節點，取代先前上下選線、左右再進入節點的方案。

後續範圍確認：使用者指出選線導覽將來可銜接快速插入節點，但目前尚無插入能力，因此本輪先不處理選線導覽。等插入功能需要時再評估新增選線搜尋方式及如何切換；不現在實作兩套搜尋或模式設定。屆時可檢視是否共用相鄰關係查詢，不能把「將來可能需要」變成本輪預先重構的理由。既有滑鼠選線、連線右鍵選單與下一輪最小命中寬度仍保留各自範圍。

最新授權與目標：使用者明確表示不必拘泥其口述的演算法／暫存結構，由開發整理邏輯；目的為流暢、符合直覺的上下左右導航，「上下是前一次的上下」。以下來源 A 的記錄是為說明預期行為，不是強制採用某種資料結構。設計以「左右沿接線前進／返回，上下切換剛才那一步的相鄰分支」為方向；候選群組不能因連按上下而漂移。在圖與位置未變、且兩次移動都未遇邊界時，下→上應回到原節點；多 input 不應使返回時突然改走無關來源。保留單選限定、邊界不循環、按鍵時才搜尋、不干擾欄位與選單等已確認限制。仍在收集本輪需求，尚未授權現在開始產品修改。

- 多選與無選取不觸發，也不自動挑起點；避開文字、數值、選單等既有鍵盤操作。不循環，沒有下一個候選時保持原選取。
- 只在按鍵時搜尋，不預先排序或維護排序清單，不要求新增連線或接孔 ID。
- 從單選節點 A 按右，找出與 A 直接連接的下游節點中位置最上方的一顆 B（Y 最小），直接選 B。
- 接著按下，仍以 A 為來源，在 A 的下游節點裡找 B 下方最近的下一顆 C；按上則反向搜尋。候選比較節點位置，不再比較線本身的高度，也不選中連線。
- 使用者再次明確要求：上／下操作不可改寫導覽來源的暫存記錄；只更新目前選取。將「目前選取」與「本次導覽基準」分開，不能透過共用選取事件把 B→C 誤記為新路徑或把 B 當成新來源。到邊界或沒有候選時，兩者皆保持不變。無記錄時的起始解析，以及圖／接線變動造成記錄失效時的清理，是獨立的初始化／有效性處理，不能每按上下就重建或替換來源。左右轉入新一段路徑或外部重新選取時如何建立／更新基準，依完整導覽規則處理。
- B／C 有多個 input 時，保留這次導覽使用的來源 A，不能每步都任意改從第一個 input 搜尋；沒有有效紀錄時，才從第一個已接線的 input 找來源。原本的來源與目前節點已無連接關係時，舊紀錄失效，將目前節點當作新的起始點。

設計整理（尚未實作）：需要保存的是本次分支導覽的基準節點 A，以及目前選取節點，而不是只有「上一次選取節點」。例如 A→B、A→C，從 B 切至 C 後 A 仍是基準；不能因 B 與 C 沒有直接連線就錯誤重設。左右沿連線推進／返回的完整對稱規則仍待整理；原先提過單選線也可啟動，但最新改為節點導覽後，既有滑鼠單選線狀態如何進入此操作尚未明確，不把它默認為刪除既有選線功能。

程式核對：現有 edge 的 `from`／`to` 已各包含節點 ID 與接孔 key，足以查詢直接相鄰節點，無須新增圖資料欄位或更改同步協定。每次按鍵掃當層 edges 並保留目前最佳候選；同一節點經多條線連接不應變成多個導覽停靠點。同高時以 X／節點 ID 等固定規則決勝為實作建議，尚未定案；不做預先排序、持續監聽或全圖遞迴。圖切換、刪線、Undo／Redo 或選取改變時須驗證／清除失效基準，不持有可錯配的舊 edge index。導覽記錄只在瀏覽器暫存，不寫入圖、Undo 或 TD。

**只有成功跳到新節點才依自動 Frame 選項處理。**

另設「方向鍵導覽自動置中」，英文候選 Frame on arrow navigation。成功跳到新選取時呼叫既有 Frame，沿用 Framing 過渡設定。用詞為 Frame，不是 Focus。此筆記與來源五輪、Colors、分類數量標籤無關；原先延後，現依 2026-09-21 補充列入下一輪。
...
## StrMenu 類文字候選（2026-09-22，作者筆記）

值仍是可自由輸入的文字，下拉選單提供常用候選，與限定合法選项的 Enum 分開。例如 Attribute 名稱可建議 `Tex`，但不限制使用者自行命名。這是後續 UI 需求與新案參考，尚未因此新增 schema 或控制項實作。舊案的補齊工作也用來從實際使用中發現具體需求。
