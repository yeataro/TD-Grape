...
- **已知問題：Subgraph 的簽名與說明容易被理解為 GLSL 函式呼叫。** 實際 `sgrape_core._expand()` 會在各使用位置遞迴展開子圖，再交由 `_compile_flat()` 產碼；不會因 Subgraph 邊界自動產生 `Tint(...)` 函式。共用定義、雙擊進入及 Make Independent 的編輯語意仍有效，後續應區分編輯器的呼叫表示與產碼行為。此項是說明問題，不是已確認的渲染錯誤。

此三項依使用者要求先記錄，不在目前 UI 修訂中新增功能。
