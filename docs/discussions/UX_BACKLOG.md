...
## 2026-09-23 Math 連續四則運算節點（介面草圖，尚未實作）

- 使用者最新確認名稱為 **Math**（語音原為 Max），將連加、連乘、連減、連除合併為一個節點，以參數頁下拉選擇；既有二元四則與取最大值 Max 保留。
- 先以對話內的可操作草圖比較「全部輸入共用運算」及「逐列選擇運算與輸入」；使用者明確要求先設計，不直接實作。
- 草圖候選：輸入 A…Z、AA、AB…；逐列使用上一列結果作左側。連減／連除按列由前至後計算，不採一般算式的乘除優先。這是供 review 的設計，尚非已確認產品規格。
- 接孔操作構想：參考 Input／Output 的動態接孔，尾端保留灰色待新增接孔，接線後自動增加輸入；此處只記錄構想，不實作。
- 這種接口暫稱 **待新增接孔（Spare Port）**：呈灰色的預留新增入口，尚不是正式輸入／輸出，接線時才建立正式接孔。這是暫定用語，本輪不更改既有 UI 標籤。
- 是否需要不同定位或分類尚未決定；具體運算規則、輸入順序與型別等細節待後續規劃。此項僅記為未來功能，不納入本次修改。
