...
搜尋 review：輸入 `mat` 時，名稱部分命中的 Matrix／mat／dmat，與別名含 matrix 的 Determinant／Inverse 被歸在同一分數，再按顯示名稱排序，因此行列式與 dmat 排在 Matrix 前。已定位為搜尋評分層級太粗；如何區分名稱前綴、名稱包含與別名命中仍待決定，本輪沒有改排序規則。
