...
> 本文為 0.8.163 的歷史盤點。2026-09-22 起的實作與原生驗證結果見 [舊案補齊紀錄](../features/LEGACY_COMPLETION.md)；下方缺口數字不可直接作為目前待辦數量。
...
2026-09-21 使用者要求：**所有內建來源及 GLSL／TD 內建函式均須有對應 Help 基本說明與官方文件章節連結**。這是現有與新增能力的共同驗收條件，完整規則見 [Alpha 範圍筆記](ALPHA_SCOPE_2026-09-21.md)。Shader／輸出資訊等不是函式的來源也包含在內，因此不能只放在函式缺口 CSV 內管理。

本表及 CSV 的「已有節點／已有來源入口」只表示入口覆蓋，**不表示 Help 已完成**；本次尚未逐項稽核所有說明與錨點。已確認的缺口是 Built-in Source：目前 Help 使用通用說明，需依具體來源提供專屬內容與連結；僅引用 TD TOP／MAT 整篇指南不構成這項驗收。
