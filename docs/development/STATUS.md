...
2026-09-18 後續順序確定：先完整整理型別與既有數學運算支援，再做 Switch Case／條件分支、UV 數字顯示等快捷功能；能力完整優先，再求簡潔 UI。數字顯示的定位選單／int 接孔／bool 開關僅為討論候選，不是強制規格，待型別完成後按便利性與通用性設計。此次更新是規劃與來源記錄，產品型別尚未擴充，詳見 [數值型別計畫](../discussions/NUMERIC_TYPES_PLAN.md)。
