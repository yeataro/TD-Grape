...
2026-09-18 Matrix 覆寫標記試行（0.8.88）：使用者同意先試收合格內的 `↳ X／Y／Z／W`，辨識該分量有獨立接線；沿用 Matrix／Column 的格子維持橫槓，分量接口仍保留，來源節點可由提示查看。15 組 Matrix UI 檢查通過，這筆視覺試行與格數修正、既有運算擴充分開提交；待交付後補記同步結果。
