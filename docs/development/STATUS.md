...
2026-09-23 0.8.222：加入 Français 與 한국어，兩處語言下拉選單共用五語清單並記住選擇，英文仍為預設。法文翻譯 About／Layout／Stage；韓文這三處保留英文。所有語系的新增節點／來源分類維持英文，面板、操作與說明按語言翻譯；韓文使用本機字型回退。新增兩語各 1481 項文字與 59 項來源提示，既有英／中／日文、技術識別碼及來源語意保持不變。

完整性／占位符／參考連結及 8 組隔離瀏覽器流程通過；確認雙選單同步、法韓持久化、語系例外、分類命名、圖／Undo／視角不變及無 API 寫入。深淺色、1133×744 面板與 430×932 窄畫面截圖已檢視；未宣稱 Safari 實機測試。39 份來源已同步，四份 Master 身分／位置及四份使用者 Shader 保留；正式 TOE 保存 1942300 bytes（SHA256 58a8b3e4a4bb83e23c471c158f4e308844a9e11faba982ce0baf702a970ebd7e），私人助手排除。私人報告 reports/locale-222/；未推送。

2026-09-23 0.8.221：About、Layout 與畫布的 VERTEX STAGE／PIXEL STAGE 標籤在中日文介面保持英文。其餘翻譯及分類規則不變。全語系檢查與 5 組語言切換回歸通過，已同步 39 份來源、四份 Master 並保留四份使用者 Shader；正式 TOE 保存 1853140 bytes（SHA256 a5bfece459603ba16b43bdbb77e7367f1a356bae1118873729764b5c3a4c83c4）。私人報告 reports/locale-221/；未推送。

2026-09-23 0.8.220：依最新確認，新增節點主／子分類及來源分類共 83 個名稱，在中文、日文介面都保留英文。面板標題、操作按鈕、提示與說明仍翻譯；分類結構、排序、節點與圖資料不變。[介面語言](../ui/LOCALIZATION.md)已記錄此例外。

1481 項文字、59 項來源提示與既有 5 組語言切換檢查通過；隔離瀏覽器核對三語的新增節點分類路徑與來源分類標籤完全相同。首次探針誤含「新增」按鈕，縮小至分類標籤後通過。39 份來源同步、四份 Master 及使用者 Shader 保留；私人佇列首次替換權限不足，重試後同步成功。正式 TOE 保存 1853156 bytes（SHA256 b4ea63d720f5ba2a0487cf2c6fcc142b559201a57c0205f056fc70a03fb25ac0）。私人報告 reports/locale-220/；未推送。

2026-09-23 0.8.219：中文面板統一為「參數」「說明」「OP 參數」「輸出預覽」「材質預覽」「面板配置」，補齊日本語並加入兩處共用語言下拉選單；英文仍為預設。新增 1481 項日文介面／節點說明及 59 項來源提示，保留技術識別碼、使用者命名與 TD 原生標籤。About、GLSL 視窗標題、Stage 說明及專案狀態提示納入切換，日文字型使用本機回退。規則見[介面語言](../ui/LOCALIZATION.md)。

全語系文字／占位符與日文參考連結檢查通過（1107 個 UI 引用）；5 組隔離瀏覽器流程核對雙選單同步、日文持久化、回切英文、圖／Undo／視角不變、無 API 寫入，以及深淺色／較窄畫面。既有英文文案與來源語意保持一致，JS 與 diff 檢查通過。39 份來源已同步，四份 Master 身分／位置及四份使用者 Shader 保留；來源 TOE 保存 1853436 bytes（SHA256 15b3f937a114e9eae54efdc138a952183befd104181a4eb22afa4f7fc932d6a8），私人助手排除。報告位於私人 reports/locale-219/；未推送。
...
39 份來源同步，四份 Master 身分／位置及四份使用者 Shader 保存內容保留。正式 TOE 保存成功（1633396 bytes；SHA256 cd7fdae6fdba72b136ed6ce7c7186c01e3bda14ff378064a039b5d141c7c1d5d），六份登記狀態保留、私人助手排除。本批本機提交後供 review，未推送。
...
2026-09-23 0.8.210：產品預設改為 Link 箭頭「常駐顯示」，Input 箭頭 hover 反向預設開啟。新瀏覽器／還原預設採用新值，既有個人偏好保留；缺少舊布林值時不執行舊值遷移。快捷鍵說明改為左右兩欄、垂直分隔，左欄為編輯，右欄為畫布與導覽及數值與文字編輯，窄視窗退回單欄。

16 組 Link 瀏覽器檢查與 8 組快捷鍵說明檢查通過，包含首次載入／還原預設、偏好遷移、雙語提示、分組與分隔線、深淺色、不同尺寸與缩放、視窗關閉及快捷鍵不穿透。私人報告在 reports/link-defaults-210/。純 UI 預設與排版修改，未重跑原生運算。

39 份來源同步，四份 Master 身分／位置及四份使用者 Shader 保存內容保留。正式 TOE 已保存（1630084 bytes；SHA256 c62b0684cebfd00588a1ade0102880f79158e0ec816131629e916bd889ff0b0d），六份登記狀態保留，私人助手排除。未推送。


2026-09-23 0.8.209：修正實驗面板排列方向。使用者所指兩列為左右並排、中間垂直分隔；左列依序放「工具列與入口」及「節點與接線」，右列放「色彩與顯示」，分組保持完整。取代 0.8.208 的上下排列。Link 三選一與偏好行為不變。

1366×768 中英文、深淺色共四組實際瀏覽器外觀檢查通過，左右位置、垂直分界與完整顯示皆確認；窄視窗保持全部控制項可達。純樣式更正，未重跑接線或原生運算。報告在私人 reports/experiments-columns-209/。

39 份來源同步，四份 Master 身分／位置及四份使用者 Shader 保存內容保留。正式 TOE 已保存（1629932 bytes；SHA256 301dab70769fd9d21264e32e04a97136cea828a6db26d64a12cfddc1e45c7cfc），六份登記狀態保留，私人助手排除。未推送。


2026-09-23 0.8.208：Link 線顯示時的導覽箭頭改為「常駐顯示／僅 hover 顯示／不顯示」三選一，預設不顯示，舊偏好自動遷移。常駐模式兩端接箭頭中心；hover 模式兩端固定接原接孔，hover 或鍵盤聚焦顯示箭頭；隱藏線時都保留導覽箭頭。實驗面板桌面排成兩列，第一列工具列與入口／節點與接線，第二列色彩與顯示；窄視窗保留自適應。詳見 [Wire／Link](../ui/WIRE_LINK.md)。

15 組 Link 瀏覽器操作、偏好遷移及重載驗證、14 項 Editor Launch、雙語鍵與 JS 語法檢查通過。1366×768 的中英文與深淺色面板皆完整顯示、不需捲動；窄視窗無水平溢出。純 UI 修改，未重跑原生運算。私人報告位於 reports/link-hover-208/。

39 份來源同步，四份 Master 身分／位置及四份使用者 Shader 保存內容保留。正式 TOE 已保存（1629908 bytes；SHA256 9ae544b8dc27afaf97ee90158e592fc91aba14e85b61ab0911f5ba2e156e7d10），六份登記狀態保留，私人助手排除。未推送；本批完成後停下供 review。
...
瀏覽器 5 組流程通過，包含四分頁、雙語標籤、鍵盤切換、Stage 篩選、未使用定義保留、個人管理區僅在個人頁、原有 TOP／MAT 範例分流。14 項 Editor Launch、JS 語法與雙語 1045 鍵通過；未改動編譯器，未重跑全套 Python。舊 browser_tree 測試同步入口名稱，未以缺少舊 fixture 的完整測試宣稱通過。報告位於私人 reports/library-tabs-203/。

39 份來源同步，四份 Master 身分／位置與四份使用者 Shader 保存內容保留。正式 TOE 已保存（1623844 bytes；SHA256 8fca472de9fabdb4259711099ee20fb21902720c1f84909088039bb40f4b2c25），六份登記狀態保留，私人助手排除。未推送。


2026-09-23 0.8.202：Subgraph 的參數與設定頁保留名稱及共用定義提示，移除進入、獨立副本／本地副本、存入個人庫三個重複按鈕。雙擊進入與個人庫拖放保留；獨立副本／本地副本移至單選 Subgraph 右鍵選單，沿用原本獨立定義與 Undo 行為，唯讀或不可修改時停用。其他節點、多選及接線不顯示此選項。

相關瀏覽器流程 4 組通過，包含兩頁名稱保留、右鍵建立獨立副本與復原、唯讀保護，以及前輪 Subgraph／來源設定回歸；JS 語法與雙語 1045 鍵通過。本輪未更動編譯器，未重跑全套 Python。報告在私人 reports/subgraph-actions-202/。

39 份來源同步，四份 Master 身分／位置及四份使用者 Shader 保存內容保留。正式 TOE 保存（1622724 bytes；SHA256 e84b198025ccc12b9eb82d36223bae90c49baba3b083e6c7ed7d83249ad175a0），六份登記狀態保留，私人助手排除。未推送。
...
39 份來源同步，四份 Master 身分／位置與三份使用者 Shader 保存內容保留；正式 TOE 已保存（1537970 bytes；SHA256 51df16c6b3852817c1c186d286a14b99c7af1b83ea678e0dc859c5a05429dc5d），五份登記狀態及私人助手排除核對通過。雙語 1047 鍵檢查通過；未推送。


2026-09-23 0.8.199：左下功能選單加入「重設瀏覽器設定…」。確認後清除本站 TD-Grape 的 21 個偏好鍵（含另存版面及舊版面記錄）並重新載入，回到目前發布預設。保留連線授權、未提交圖草稿、其他網站應用資料與 TD／TOE 狀態。未完成欄位／寫入中阻止執行，草稿保存失敗不清設定，設定清除失敗還原已移除項目。6 組隔離瀏覽器檢查通過，包含真實 reload、回到預設、雙語與沒有 TD 寫入。詳見[版面配置](../ui/WORKSPACE_LAYOUT.md)。

39 份來源同步，四份 Master 身分／位置、三份使用者 Shader 保存內容保留；正式 TOE 已保存（1536642 bytes；SHA256 28c5cf93e8101b7f9d49f5ffa4d3e815ecca8ab7e0b76dfb6ad34160a8b15fee），五份登記狀態及私人助手排除核對通過。雙語 1050 鍵與 JS 語法通過。額外 applied-graph reload 舊測試未先打開功能選單，直接點隱藏按鈕逾時；HEAD 亦為同一 popover 入口，該測試未計入通過。新 reset 測試透過實際左下選單操作，未推送。
...
39 份來源同步、四份 Master 保存身分／位置及原生參數，兩份使用者 Shader 保存內容比對不變。TOE 已保存（1326312 bytes；SHA256 817820c434242ed0ef85d10b2cc877b13c0cf3ae7e5bd284356389babc07067c），私人助手排除。未執行發布或推送。進階參數定義仍在 TD 編輯；觸控與 macOS 未新增實測。

2026-09-23 0.8.196：Group 標題恢復在线條之上；Group、Node、Wire／Link 的 Ctrl／Shift 點選統一追加與取消，Shift 拖曳保留框選。Group 依成員整組加入／移除，部分選取時補齊。多條接線共用高亮，右鍵保留已選集合，樣式轉換及斷開連接共用一次 Undo；來源／目的與來源接孔動作仍作用於右鍵線。接線選取僅存在當前快照，刪除前序線不會錯選其他線，失敗編輯可恢復選取。TD 文件未明示 Shift 點選與 Ctrl 相同，此同義行為由作者另外確認。

相關瀏覽器測試共 81 組通過（新增多選 9、畫布 7、Group 13、接線命中 9、批次樣式 4、Link 導航 12、端點 Frame 3、選取快捷 14），雙語 1053 鍵通過。旧測試明確關閉不屬該測試範圍的畫布阻尼、鍵盤選單以鍵盤操作、英文 fixture 以英文驗證；圖層斷言依新裁定更新。額外垃圾桶回歸在第二指取消拖曳時失敗，修改前 HEAD 的四份原始 UI 檔案覆蓋測試亦在同一位置失敗，未改動該既有問題；前面的滑鼠／觸控拖曳與取消檢查已通過，不能宣稱整份垃圾桶測試通過。報告位於私人 reports/multi-selection-196/。

39 份來源同步，四份 Master 人工內容／身分／位置及三份使用者 Shader 保留（Phong revision 473，PBR revision 157）。TOE 保存（1412560 bytes；SHA256 a5d97fb577dd24478058f55909489b9259f52ef86972476356d3ecc20d034b3c），五份登記狀態保留、私人助手排除，未推送。

2026-09-23 0.8.195：線條右鍵在選取來源／目的下新增「來源全部轉 Link」，只處理同一來源接孔的所有線；節點單選／多選新增「轉換接線」二級選單，依序輸入轉 Link、輸入轉 Wire、輸出轉 Link、輸出轉 Wire。批次修改共用一次 Undo／Redo，無需變更或唯讀時停用，圖／選取改變後的過期選單不執行。排列與轉換共用二級選單的定位、滑鼠與鍵盤行為。新增流程 4 組（含單選／多選四項组合、精確接孔範圍、Undo／Redo、唯讀／過期與排列回歸）、Link 箭頭選單 4 組、既有導航 12 組與雙語檢查通過。39 份來源同步，四份 Master 身分／位置／人工內容及兩份使用者 Shader 保留（Phong revision 441，PBR revision 100）。TOE 保存（1323592 bytes；SHA256 dd6a5e6b9584092330a2466ab7da0414f8bd480f12568143d7debc630f06d8d7），四份登記狀態保留、私人助手排除，未推送。

2026-09-23 0.8.194：隱藏 Link 線時，箭頭右鍵清單加入左側 3 × 18px 類別色條，沿用新增節點配色，文字維持原色；分隔線下方提供「全部改回 Wire」，一次轉換該箭頭代表的所有 Link。一般／合併接孔作用範圍、其他接線與 UI 保留、一次 Undo／Redo、唯讀與過期選單、雙主題色條 4 組及既有 Link 導航 12 組通過；雙語檢查通過。39 份來源同步，四份 Master 身分／位置／人工內容及兩份使用者 Shader 保留（Phong revision 440，PBR revision 30）。TOE 保存（1322640 bytes；SHA256 45db2ec1552d2f324aa0c4275829664d90151cfff6e81c50a28ce083cf68ca07），保留四份登記狀態、私人助手排除，未推送。

2026-09-23 0.8.193：實驗選項新增「選取接線來源／目的時自動 Frame」，預設關閉。啟用後 Wire／Link 右鍵選取對端呼叫一般 Frame，動畫與時間沿用 Frame 過渡；Link 箭頭導航不變。設定只存瀏覽器。新流程 3 組（含 16 種來源／目的、Wire／Link、開關／動畫组合、唯讀與偏好保存）、一般 Frame 7 組、Link 導航 12 組及雙語檢查通過。一般 Frame 舊測試的動畫預設關閉假設已更新為目前預設，未更改產品預設。39 份來源同步，四份 Master 身分／位置／人工內容及兩份使用者 Shader 保留（Phong revision 439）。TOE 保存（1320088 bytes；SHA256 17b0a2a308c7f15636b3bb5043eebed61f9817c67be3dcabc148145d52462f15），私人助手排除，未推送。

2026-09-23 0.8.192：Structures／結構面板與 GLSL 同樣預設隱藏，套用於首次使用與還原預設，既有個人配置保留。面板瀏覽器 6 組通過，包含舊版面保留已顯示 Structures 的偏好，以及還原預設後兩面板皆隱藏。39 份來源同步，四份 Master 身分／位置／人工內容及兩份使用者 Shader 保留（Phong revision 437）。TOE 保存（1319824 bytes；SHA256 c70ffe51b361b6d806bb4d03f085b3ccd24f7d656de1ea369dcff2d60497ea39），私人助手排除，未推送。

2026-09-23 0.8.191：新增預設隱藏的 GLSL 工作區面板，位置為右上 Parameter／Custom Parameters 同組分頁，從 Layout 面板選單開啟。只顯示目前階段產碼，可選取／複製／捲動，沿用上下分隔把手；沒有字級或額外內容按鈕，也不需要畫布顯示器。共用既有顯示器快取與更新機制，非作用分頁不額外產碼；舊配置只補入隱藏分頁。面板 5 組、畫布顯示器 9 組、版面配置 7 組瀏覽器檢查與雙語檢查通過。39 份來源同步，四份 Master 身分／位置／人工內容及兩份使用者 Shader 保留（Phong revision 436）。TOE 保存（1319816 bytes；SHA256 276483308d5736ccd5cb686fdc0a790447980e734edcc8b4d34d95585f5cc2b1），私人助手排除，未推送。詳見 [Generated GLSL](../ui/GENERATED_GLSL.md)。

2026-09-23 0.8.190：修正 0.8.189 將 GLSL 顯示器黑色直接填滿的解讀；現在與 Note 共用主題混色及語法配色，不再有黑色專用樣式。調色盤移除額外黑色色票，只將左上角預設選色換為黑色，其餘排列與 Note 相同；Note 預設不變。9 組顯示器瀏覽器檢查通過，含深／淺色下與 Note 實際底色比較、色票數量與預設還原；雙語檢查通過。39 份來源同步，四份 Master 身分／位置／人工內容及兩份使用者 Shader 保留（Phong revision 413）。TOE 保存（1319736 bytes；SHA256 8de06b7a5341821a25262d9b35e3912253cc55760d6dd2562a21ec114533ae3a），私人助手排除，未推送。

2026-09-23 0.8.189：Generated GLSL 預設改為純黑背景，專用調色盤增加純黑；深／淺色主題皆保持純黑與可讀語法色。寬高上限各放寬至 3000 圖單位，字級倍率為 0.1–10；Note 仍為 1–10，既有大小與顏色行為維持。顯示器瀏覽器 9 組、Note 外觀 16 組通過；雙語檢查通過。39 份來源同步，四份 Master 身分／位置／人工內容及兩份使用者 Shader 保留（Phong revision 371）。TOE 保存（1319648 bytes；SHA256 dbd49de343126255e58500b8426556a8150be00e39d90c649fd49b2c9e068786），私人助手排除，未推送。

2026-09-23 0.8.188：新增 Generated GLSL 唯讀顯示節點，每個 Vertex／Pixel／子圖畫布各一個，顯示所在階段完整 GLSL，沿用 Note 外觀、標題高度、折疊、雙向大小及顏色／字體設定。產碼本文只在記憶體／DOM，隨語意編輯更新；錯誤清楚取代舊碼，延遲回覆不跨圖／階段套用。新增、複製／貼上、匯入及編譯驗證皆遵守唯一限制。詳見 [Generated GLSL](../ui/GENERATED_GLSL.md)。

完整 portable 通過（532 Python、14 integration、1048 雙語鍵及既有 JS suites）；首次三項失敗為舊節點基準排除清單未包含新節點，補上排除後全部通過，138 份原產碼指紋未修改。顯示器 8 組、Note 8 組、外觀 16 組、雙向調整大小 14 組瀏覽器檢查通過。TD 原生 validate 入口核對 TOP／MAT／Phong／PBR 四份 Master 的產碼、指紋、診斷及來源映射均不受顯示器影響，沒有 Apply 測試圖。39 份來源同步；刷新逐一替换 DAT 期間記錄一次新表／舊 core 的 Unknown catalog identity，完整載入後 Master 同步及四份原生檢查通過。四份 Master 身分／位置／人工內容及兩份使用者 Shader 保留（Phong revision 338）。TOE 保存（1319408 bytes；SHA256 9dfeb67cff8a8635a95db21c68e20e0ead9d40ce526981aba0353947d708499b），私人助手排除，未推送。

2026-09-23 0.8.187：折疊節點的合併 Link 箭頭依實際 Link 接孔型別顯色：同型別保留型別色，異型別使用低彩度左上→右下彩色漸層，不納入普通 Wire 與未接孔。保留常態 60% 不透明度、hover 與左右鍵導航。私人探針 6 組（雙端、同型別、同色異型別、排除 Wire／未接孔、動態更新／展開／主題／顯隱及導航）與既有 Link 回歸 12 組通過。39 份來源同步，四份 Master 人工內容／身分／位置及兩份使用者 Shader 保留（Phong revision 314）。TOE 保存（1335112 bytes；SHA256 efcd0026674379ded3f648efe7ac21b1dcfc1a907d29d880a5230413c70ef17b），私人助手排除，未推送。

2026-09-23 0.8.186：「顯示自訂名稱／顯示 Link 線」工具列按鈕共用「還原介面」的底色與普通框線，啟用只以紫色文字表示，移除選中綠底、紫框及光暈。私人瀏覽器探針核對深色／淺色 Cool 與深色 Godlike 的實際底色、框線、文字及 hover，兩按鈕真實點擊與偏好保存通過（4 組）。39 份來源同步，四份 Master 身分／人工內容／位置與兩份使用者 Shader 保留（Phong revision 286）。TOE 保存（1315584 bytes；SHA256 5eed510e1136c0a59a5d7bdc549910c7816a94f61b17a58cf32d852ba22e2bc6），私人助手排除，未推送。

2026-09-23 0.8.185：接線右鍵選單移除「顯示 Link 線」，保留 Wire／Link 樣式及接線操作；全畫布顯示開關仍在畫布右鍵、工具列及 X。12 組 Link 瀏覽器檢查通過。39 份來源同步，四份 Master 人工內容／身分／位置及兩份使用者 Shader 保留（Phong revision 245）。TOE 保存（1315432 bytes；SHA256 ab1ac9eb107582b07a5f19a21f9b5bbb4fe60243a0c1766e0af83d048c614997），私人助手排除，未推送。

2026-09-23 0.8.184：隱藏 Link 時的接孔導航箭頭改為 SVG，共用一般 Wire 的型別／分量色與 2.5px 線寬，常態圖示不透明度 60%、hover 100%。按鈕底色與框線採節點 Body／外框樣式，輸入端箭頭改朝右；左右鍵導航不變。12 組 Link、9 組 wire hit area 回歸通過；另以私人探針比對深／淺色的實際框線、底色、Wire 色／線寬及 hover 不透明度。39 份來源同步，四份 Master 人工內容／身分／位置及兩份使用者 Shader 保留（Phong revision 244）。TOE 保存（1315448 bytes；SHA256 6c77377162b3400b6abcbb06b3cf94130797881352f39aa970046471e77ddc89），私人助手排除，未推送。

2026-09-23 0.8.183：Link 常態線寬 1.25→1px、灰色不透明度 60%；hover 線寬 1.5px、顏色不透明度 65%，中央三角形同步淡化。選取與錯誤顯示保持。12 組 Link 瀏覽器回歸通過，39 份來源同步，四份 Master 人工內容／身分／位置及兩份使用者 Shader 保留（Phong revision 211）。TOE 保存（1315104 bytes；SHA256 c1d5b6f5db50df380651e80179c77155e6699a040f23b8347aff537454d20e22），私人助手排除，未推送。

2026-09-23 0.8.182：依作者新裁定，取代接孔箭頭常駐的行為：Link 線顯示時只在中央畫來源→目的三角形，接孔導航箭頭僅於隱藏 Link 線時出現。箭頭底色與節點 Body 相同，hover 加強辨識，左右鍵導航保持。線、三角形及接孔導航箭頭沿用原生懸停提示，多行列出來源／目的節點與接孔；接孔提示包含所有 Link，排除 Wire。Wire／Link／顯示 Link 線的右鍵項目改為左側勾選標記，移除加號及選中底色／文字高亮，保留 hover。

12 組 Link 與 9 組既有 wire hit area 瀏覽器檢查通過，涵蓋三角形方向／命中／選取／右鍵、雙主題背景、提示內容、勾選狀態、切換與導航；1041 雙語鍵及修改 JS 語法檢查通過。39 份來源同步，四份 Master 人工內容／身分／位置及兩份使用者 Shader 保留；Phong Master revision 208。TOE 保存（1315088 bytes；SHA256 24b41e955050006f53dff2ebe5cacb5b362506bb764a45f91ca7d53c813a8093），私人助手排除，未推送。

2026-09-23 0.8.181：顯示 Link 線移出實驗功能，在畫布「顯示自訂名稱」同組新增按鈕，畫布／接線右鍵選單與快捷鍵表加入 X。三個入口共用顯示偏好；隱藏線及命中區仍保留箭頭與導航。獨立保存並承接 0.8.180 的舊偏好，重設實驗功能不影響它。文字輸入、對話框與畫布手勢不觸發 X，按住不反覆切換。

10 組 Link 瀏覽器檢查、7 組工具列溢出、7 組快捷鍵表檢查通過，含窄畫面、唯讀、無 Undo 副作用、重新載入及偏好遷移；1041 雙語鍵及修改 JS 語法檢查通過。39 份來源同步，四份 Master 人工內容／身分／位置及兩份使用者 Shader 保留；Phong Master revision 187。TOE 保存（1314392 bytes；SHA256 7af04ab22f6955ce2c298f01da29f0b14c7978ce57970d5c741cb61239ac0a16），私人助手排除，未推送。
...
2026-09-21 0.8.163 DEV：依使用者截圖 review 補上 Frame 無動畫，視角共五項；立即置中並縮放，不受一般 Frame 動畫開關影響。TD 內建 GLSL 函式缺口調查列為下一輪優先，先整理待辦，不擴張本次產品修改。

2026-09-21 0.8.162 交付檢查點：程式 ea63cee、待辦整理 1677ed4；92 組相關瀏覽器檢查及完整 portable checks（510 Python）通過，阻尼首次並行執行的時序失敗與單獨重跑結果已如實記於 TESTING。37 份內嵌來源、11 份實際 HTTP 資產一致，core 無錯誤；TOP／MAT master revision 109，四份使用者 Shader 保留。TOE 1028526 bytes，SHA256 71069ca3517920d167cd9b373f3f6f0fdd9c16f6141bb7734d04ba067947b8f1，排除一份私人助手。報告 navigation-review-162/。已提醒可刷新，未重新載入使用者瀏覽器。

目前待辦與舊筆記覆核見[待辦總整理（2026-09-21）](../discussions/TODO_AUDIT_2026-09-21.md)。下方日期較早的「目前待辦／下一輪」保留歷史背景，不能覆蓋更新的交付紀錄。

2026-09-21 0.8.162 DEV review 修正：Ctrl＋左右的一階實驗改為增量選取，保留單選／多選及孤立起點，每次只加入一階，下一次才繼續向外；未開啟仍全選上游／下游。方向鍵預設改為依畫面位置，其他兩版保留。視角改為不移動、Frame 有動畫、Center 有動畫、Center 無動畫四選一；Center 為獨立的只平移能力，保持當前倍率，Frame 保留縮放。導覽動畫獨立於一般 Frame 開關，H／F 規則保留；舊自動 Frame 偏好映射至新選項。驗證與同步結果另記。

2026-09-21 0.8.161 交付檢查點：程式 9145780；91 組瀏覽器檢查（導覽 20、接線選取 14、設定 20、Help 7、排列 15、命中 9、全螢幕／專注 6）及完整 portable checks（510 Python）通過。37 份內嵌來源及 11 份實際 HTTP 資產一致，core 無錯誤；TOP／MAT master revision 108，四份使用者 Shader 保留。TOE 1028134 bytes，SHA256 4d824a47582a12b9c058c14a01653349b28bf17c26093dcf29692deeaf80f519，私人助手排除。報告 arrow-branches-161/；未重新載入使用者瀏覽器。Mac 實機尚未驗證。

2026-09-21 0.8.161 補充：Ctrl＋Enter 切換專注編輯，Esc 僅取消不退出；文字提交與 Alt＋Enter 全螢幕保留。快捷鍵、實驗選項及相關提示自動顯示作業系統的 Ctrl／Cmd，新增命令接受 Control／Meta，移除 Mac 例外段落。Mac／Safari 實機驗證保留待辦，本輪僅模擬平台資訊。

2026-09-21 0.8.161 DEV：實驗功能新增方向鍵三版本（原版路徑／雙向分支／依畫面位置），預設雙向分支，返回後仍可切換剛經過節點的其他上游。另新增 Ctrl＋左／右全部上游／下游（含起點），Ctrl＋上全部相連，Ctrl＋下為當前圖中相連區塊的補集。可開啟獨立的「Ctrl＋左右只選下一階」，預設關閉；多選取各起點聯集，無下一階時保留選取，Ctrl 上下不受此選項影響。L／Shift＋L 排列保留。快捷鍵 Help 隨模式更新；設定僅存瀏覽器，選取不修改圖／Undo／TD；保持單顆方向導覽只接受單選，不循環、不預排序。

2026-09-21 0.8.160 交付檢查點：程式 7a20d67；9 組選線／層級檢查及完整 portable checks 通過。37 份內嵌來源、11 份實際 HTTP 資產一致，core 無錯誤；TOP／MAT master revision 107，四份使用者 Shader 保留。TOE 1026022 bytes，SHA256 522ff2cdf00b487084f9c9cee08bb661692ba178ea1f6c8e766584ff0a051293，私人助手已排除。報告 group-title-160/；未重新載入使用者瀏覽器。
...
驗證：93 組隔離瀏覽器檢查、510 Python、14 ancillary、26 Node 與完整 portable checks 通過；TD 原生 19 組，含 200 個來源／100 次更新無逐次清單掃描或編譯，以及同路徑同值 master 重建拒絕 REST／既有手勢；真正 Editor→TD loopback／私人網路 16 組，含延遲真實編譯回覆、持續拖曳、REST 排隊、舊 HTTP、連續鍵入、重開及圖／值 Undo、Redo。測試使用可清除元件，核對使用者 Shader 保留。報告 Documents 工作區 reports/uniform-boundaries-155/、私人 work/reports/uniform-boundaries-155/。研究及實作界線見 docs/discussions/UNIFORM_SYNC_BOUNDARY_STUDY.md。同步保存另記。

2026-09-20 0.8.154 DEV 交付檢查點：程式提交 `6c8b166`；37 份嵌入來源與 10 份服務資產一致、core errors 為空，TOP／MAT master revision 101。目前三份使用者 Shader 在來源刷新、模板同步與保存前後均保留。TOE 984462 bytes，SHA256 `323397b4a03f5a77d1ce0f3f383f26875a71da7506d3cf385dd738b2ccd24fdf`；私人開發助手已由保存工具排除。報告 `work/reports/ui-notes-154/`。未重新整理使用者現有網頁，待使用者自行載入新版。
...
2026-09-20 0.8.142 DEV 交付檢查點：程式提交 `c74befc`（含 0.8.141 名稱切換 `8b90606`）；37 份嵌入來源、10 份服務資產一致，TOP／MAT master revision 89，三份使用者 Shader 保留。TOE 981598 bytes，SHA256 `bf01e6c488119c40a3794d5fb8e892f103e53a748ed313f9ee8a47af7e792a21`。報告 `work/reports/source-fixes-142/`。使用者網頁未自動重新整理。

2026-09-20 來源與 UI 0.8.142 DEV：修正初始化配方被當作來源身分造成的共同來源衝突，清單／再次引用／名稱共用辨識；外部 Parameter Bind 與多層綁定可編輯，保留綁定與 Undo，Bind 紫色數值、Master 角標、Constant 亮度。卡片系統預設備註／名稱換行省略／選取同步、單群組取消收合、Appearance 移到主題切換上方，以及左下角分組功能表均完成。0.8.141 名稱切換一併交付。507 項 Python、原生來源 23 與即時通道 14 組驗證通過；瀏覽器來源身分 6、卡片 8、原生分量 7、Colors 5、即時通道 10、共用選單 11、功能表 4、卡片／面板幾何 4 組通過；既有選取 7、工作區配置 7 組通過。長語言清單沿用既有上拉規則並驗證。型別／名稱衝突與 driven master 保護仍保留。移動節點暫鎖 Uniform 的變更範圍問題已依使用者要求另列後續；同步保存另記。

驗證界線：舊 workspace_regression 的創建器接線段在 0.8.141 基準同樣失敗，前五組工作區檢查通過；另以本輪專用幾何／折疊檢查驗證面板修正，不宣稱整套舊工作區腳本通過。並行執行的兩個手勢腳本偶有時序逾時，獨立重跑 select_menus 與 uniform_live 均通過。原生新增測試只建立並移除自己的隔離元件，使用者三份 Shader 保留。

2026-09-20 共同來源名稱 0.8.141 DEV：新增 TD Names／Common Names，依來源表的明確顯示名稱切換清單與畫布標題，保留分類／圖／Undo／數值控制 DOM；兩套名稱可搜尋，偏好可保存。共同來源簡短 hover 說明與 Output Info 類別名稱已補。来源表 7、來源展示 4、卡片 8、分類 3 組回歸通過；名稱／搜尋／窄面板／保存 3 組檢查與截圖核對通過。內建卡片點擊／展開與 Buffers 分類仍為討論筆記。同步保存另記。
