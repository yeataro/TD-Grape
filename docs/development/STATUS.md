...
下一輪分 UI 與型別兩批：先整理 Inputs 分類新增／引用拖放／緊湊色塊清單、工具列分組與響應式尺寸、頂欄收合／About／Refresh，以及底部狀態；再補一般數值型別與 Specialization Constants。Array／Matrix 希望至少型別與合理初始化可用，初版範圍仍需評估，完整值編輯介面後補。詳細實作清單、後續待辦和模糊筆記統一見 [Inputs 面板與工作區設計](../discussions/INPUTS_UI_NEXT_ROUND.md)。本次僅整理規劃，尚未啟動上述兩批實作。UI 亮度、空白畫布的設定頁、Vector I/O 拆分與資源庫獨立 panel 不預先定案。
