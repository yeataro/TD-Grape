...
function compositeStructs(document=activeTypeDocument()){
  const result={...(typeContract?.composites?.structs||{})};
  for(const item of document?.typeDefinitions||[])result['struct:'+item.id]=item;
  return result;
}
function typeDescriptor(type,document=activeTypeDocument(),seen=new Set()){
  if(typeof type!=='string'||type.length>512||seen.has(type)||(type.match(/\[/g)||[]).length>8)return null;
  if(typeContract?.types&&Object.hasOwn(typeContract.types,type))return typeContract.types[type];
  const array=/^([^\[\]]+)\[([A-Za-z_][A-Za-z0-9_]*|[0-9]+)\](.*)$/.exec(type);
  if(array){
  ...
  const definitions=compositeStructs(document),definition=Object.hasOwn(definitions,type)?definitions[type]:null;
...
function arrayLengthLabel(length,document=activeTypeDocument()){
  const declaration=arrayLengthDeclaration(length,document);
  return typeof length==='number'?String(length):declaration?.kind==='constant'?String(declaration.value):'N';
}
function arrayType(elementType,length){const at=elementType.indexOf('[');return at<0?elementType+'['+length+']':elementType.slice(0,at)+'['+length+']'+elementType.slice(at);}
function compositeValue(value,type,document=activeTypeDocument(),budget={remaining:65536},active=new Set()){
  ...
  return shapedValue(value??0,type);
}
const compositeZeroValue=(type,document=activeTypeDocument())=>compositeValue(undefined,type,document);
function typeAvailable(type,document=activeTypeDocument(),seen=new Set()){
  if(seen.has(type))return false;seen.add(type);const descriptor=typeDescriptor(type,document);if(!descriptor)return false;
  const target=typeof editorTarget!=='undefined'?editorTarget:document?.target||'top',shaderStage=typeof stage!=='undefined'?stage:'pixel';
  if(descriptor.shape==='array')return typeAvailable(descriptor.elementType,document,seen)&&(typeof descriptor.length==='number'||!!arrayLengthDeclaration(descriptor.length,document)||Object.values(typeContract?.composites?.sources||{}).some(source=>source.type?.includes('['+descriptor.length+']')&&(source.targets||[]).includes(target)&&(source.stages||[]).includes(shaderStage)));
  return descriptor.shape!=='struct'||((descriptor.targets||['top','mat']).includes(target)&&(descriptor.stages||['pixel','vertex']).includes(shaderStage)&&(descriptor.fields||[]).every(field=>typeAvailable(field.type,document,new Set(seen))));
}
function displayType(type,document=activeTypeDocument()){
  const descriptor=typeDescriptor(type,document);if(descriptor?.shape==='array')return arrayType(displayType(descriptor.elementType,document),arrayLengthLabel(descriptor.length,document));
  return descriptor?.shape==='struct'?descriptor.name||type:type;
}
function glslTypeName(type){
  const descriptor=typeDescriptor(type);if(descriptor?.shape==='array')return arrayType(glslTypeName(descriptor.elementType),arrayLengthExpression(descriptor.length));
...
function glslOutputInitialization(name,type){
  const lines=[];let counter=0;
  const emit=(name,type,indent)=>{const descriptor=typeDescriptor(type);if(descriptor?.shape==='array'){
    const index='sg_init_'+counter++;lines.push(indent+'for (int '+index+' = 0; '+index+' < '+arrayLengthExpression(descriptor.length)+'; ++'+index+') {');emit(name+'['+index+']',descriptor.elementType,indent+'    ');lines.push(indent+'}');
  }else lines.push(indent+name+' = '+glslZeroLiteral(type)+';');};emit(name,type,'    ');return lines.join('\n');
}
function graphInterfaceTypes(document=activeTypeDocument()){
  const result=new Set([...interfaceTypes(),...Object.keys(compositeStructs(document))]);
  const add=type=>{if(typeDescriptor(type,document))result.add(type);};
  ...
  for(const f of document?.functions||[])for(const p of [...f.inputs,...f.outputs])add(p.type);
  return [...result].filter(type=>typeAvailable(type,document));
}
function typeContainsResource(type,document=activeTypeDocument(),seen=new Set()){
  if(seen.has(type))return false;seen.add(type);if(isResourceType(type))return true;
  const descriptor=typeDescriptor(type,document);
  return descriptor?.shape==='array'?typeContainsResource(descriptor.elementType,document,seen):descriptor?.shape==='struct'?(descriptor.fields||[]).some(field=>typeContainsResource(field.type,document,seen)):false;
}
...
function compositePorts(key,params,document=activeTypeDocument(),incoming={}){
  ...
  if(key==='struct_field'){
    ...
    return {inputs:{value:type},outputs:{out:field?.type||'?'}};
  }
  const type=incoming.Array||params.type||'float[4]',descriptor=typeDescriptor(type,document),inputs={Array:type};
  if(descriptor?.shape!=='array')throw Error(t('composite.arrayRequired'));
  if(key==='array_replace'&&typeContainsResource(type,document))throw Error(t('composite.resourceReadOnly'));
  if(key==='array_length')return {inputs,outputs:{out:'int'}};
  inputs.i=params.indexType||'int';if(key==='array_replace')inputs.replacement=descriptor?.elementType||'?';
  return {inputs,outputs:{out:key==='array_replace'?type:descriptor?.elementType||'?'}};
}
...
const CustomGLSL=(()=>{
  ...
  function validate(params){
    ...
    for(const direction of directions){
      ...
      for(const p of list){
        ...
        if(!(direction==='inputs'?graphInterfaceTypes():graphValueTypes()).includes(p.type))throw Error(t('code.invalidType'));
  ...
  function update(node,direction,id,patch){
    ...
    if(direction==='inputs'&&before!==p.type&&Object.hasOwn(node.inputValues||{},id)){
      if(!hasValueEditor(p.type))delete node.inputValues[id];
  ...
  function header(params){
    const signature=directions.flatMap(direction=>params[direction].map(p=>`${direction==='inputs'?'in':'out'} ${glslPortDeclaration(p.type,p.name)}`)).join(', ');
    const defaults=params.outputs.map(p=>glslOutputInitialization(p.name,p.type)).join('\n');
...
function setTypeContract(contract){
  ...
  const known=type=>{
    if(valueTypes.includes(type)||resourceTypes.includes(type)||typeof type==='string'&&Object.hasOwn(contract.composites?.structs||{},type))return true;
  ...
  for(const entry of Object.values(contract.definitions)){
    if(!['fixed','parameter','declaration',...(contract.composites?['dynamic']:[])].includes(entry.selector)||!Array.isArray(entry.variants)||!entry.variants.length)throw Error(t('contract.invalid'));
...
function typeComponents(type){
  const descriptor=typeDescriptor(type);if(!descriptor)throw Error(t('contract.invalid'));
  return descriptor.components||0;
}
function shapedValue(value,type){
  if(isResourceType(type))return null;
  if(isCompositeType(type))return compositeValue(value,type);
...
function typeVariants(d){
  ...
  const entry=typeContract?.definitions[d.definitionUuid];if(entry)return d.fixedType?entry.variants.filter(v=>v.type===d.fixedType):entry.variants;
...
function resolvedNodePorts(d,params,decl,kind){
  ...
  if(entry?.selector==='declaration'&&decl&&isCompositeType(decl.type))return kind==='inputs'?{}:{out:decl.type};
...
function reshapeTypedInputs(n,d,nextType,nextOperands=null){
  ...
  if(isCompositeOperation(d)){
    ...
    if(d.key==='struct_field'){
      const fields=typeDescriptor(nextType)?.fields||[];if(!fields.some(field=>field.id===n.params.field||field.name===n.params.field))n.params.field=fields[0]?.id||fields[0]?.name||'';
    }
    if(n.inputValues?.replacement!==undefined){const type=typeDescriptor(nextType)?.elementType;if(hasValueEditor(type))n.inputValues.replacement=shapedValue(n.inputValues.replacement,type);else delete n.inputValues.replacement;}
    return;
  }
...
function creatorVariants(d,wire){
  if(isCompositeOperation(d)){
    ...
    if(d.key==='struct_field'){
      if(!wire&&!typeAvailable(params.type)){params.type=Object.keys(compositeStructs()).find(type=>typeAvailable(type));params.field=typeDescriptor(params.type)?.fields?.[0]?.id;}
      const structures=wire?.kind==='outputs'?(descriptor?.shape==='struct'?[wire.type]:[]):wire?.kind==='inputs'?Object.keys(compositeStructs()):[params.type];
      for(const type of structures)for(const field of typeDescriptor(type)?.fields||[])if(!wire||wire.kind==='outputs'||compatible(field.type,wire.type))add({...params,type,field:field.id||field.name});
      return variants;
    }
    if(wire&&descriptor?.shape==='array')params.type=wire.type;
    else if(wire?.kind==='inputs'&&d.key==='array_get'){
      const arrays=graphInterfaceTypes().filter(type=>typeDescriptor(type)?.shape==='array'&&compatible(typeDescriptor(type).elementType,wire.type));
      for(const type of arrays)add({...params,type});if(!arrays.length&&!isResourceType(wire.type))add({...params,type:wire.type+'[4]'});return variants;
    }
    add(params);return variants;
  }
...
function creatorPriority(match,wire){
  ...
  return index<0?99:index+(match.d.fixedType&&match.portType!==wire.type?0.5:0);
}
...
function inputTypeDisplay(n,port){
  ...
  return {source:source||null,target,conversion,text:source&&source!==target?displayType(source)+' → '+displayType(target):displayType(target)};
}
function portTypeCaption(n,kind,name){
  if(kind==='outputs')return el('small',{},displayType(ports(n,kind)[name]));
...
function nodePreferredWidth(card){
  ...
  for(const input of probe.querySelectorAll('input:not([type=color])')){
    ...
    // Numeric precision is editable content, not a change to the card's layout.
    const sample=input.classList.contains('numeric-slider')?'0.000000':input.value||'0';
    input.style.width=Math.ceil(Math.max(42,context.measureText(sample).width+padding+8))+'px';
  }
...
function renderNodeCard(n,cards,nativeDeclarations,projection=null){
    ...
    const displayName=projection?.name||nodeDisplayName(n),canvasTitle=displayName;
    ...
    const portRow=(kind,name,missing=false,compact=false)=>{
      ...
      if(d?.key==='vertex_out'&&kind==='inputs'&&name==='position'){
        b.title=row.title=t('vertex.positionHint');
        if(!compact)row.classList.add('vertex-position-port');
      }
...
function openCreator(clientX,clientY,wire=null,{place=false}={}){
  if(readonly||!graph)return;cancelNodePlacement();cancelConnection();creatorDragGesture?.cancel();const position=graphPoint(clientX,clientY);if(!position)return;
  creatorState={path:[],anchorX:clientX,anchorY:clientY,...position,wire,place,owner:graph,level:current(),stage,version:editVersion};creatorIndex=0;
  const typeFilter=$('#createtype');typeFilter.replaceChildren(el('option',{value:'all'},t('create.allTypes')),...graphInterfaceTypes().map(type=>el('option',{value:type},displayType(type))));
...
function chooseCreator(index){
  ...
  if(graph!==state.owner||current()!==state.level||stage!==state.stage||editVersion!==state.version){closeCreator();return;}
  const locked=$('#createtype').value!=='all';
  if(state.place){beginNodePlacement(match,state,locked);return;}
  const changed=commitCreatorMatch(match,state,locked);
  if(changed){closeCreator();$('#canvas').focus();}else $('#createsearch').focus();
}
function materializeCreatorMatch(match,state,locked,preview=false){
  ...
  const result=instantiate(d,state.x,state.y,match.type,{locked,declarationId:match.d.inputSourceId,inputSeed,preview}),n=preview?result.node:result;
  Object.assign(n.params,clone(match.params||{}));
  if(state.wire&&isVectorOperation(match.d)){const peer=current().nodes.find(p=>p.id===state.wire.node);n.ui.componentNames=nodeComponentNames(peer);}
  return result;
}
function commitCreatorMatch(match,state,locked){
  return change(()=>{
    const n=materializeCreatorMatch(match,state,locked);
    if(state.wire){
...
function cancelNodePlacement(){nodePlacement?.cancel();}
function beginNodePlacement(match,state,locked){
  if(editorMutationBlocked())return;
  cancelNodePlacement();cancelValueLadder();
  const measurement=el('div',{class:'node-placement-measure','aria-hidden':'true'});measurement.inert=true;$('#world').append(measurement);
  let size;
  try{
    const projection=materializeCreatorMatch(match,state,locked,true),n=projection.node;
    projection.ports=match.variant;projection.connectedInput=state.wire.kind==='outputs'?match.port:null;
  ...
  }catch(error){status(error.message,true);return;}finally{measurement.remove();}
  closeCreator();closeGraphMenu();focusGraphCanvas();
  const layer=el('div',{class:'node-placement','aria-hidden':'true'}),outline=el('div',{class:'node-placement-outline'}),title=el('div',{class:'node-placement-title'},size.name),socket=el('span',{class:'node-placement-port'});
  const svg=document.createElementNS('http://www.w3.org/2000/svg','svg'),wire=document.createElementNS(svg.namespaceURI,'path');svg.append(wire);layer.append(svg,outline);outline.append(title,socket);$('#world').append(layer);
  outline.style.width=size.width+'px';outline.style.height=size.height+'px';title.style.height=size.title+'px';socket.style.left=(size.port.x-1.5)+'px';socket.style.top=(size.port.y-1.5)+'px';
  const controller=new AbortController(),options={capture:true,signal:controller.signal},owner=graph,level=current(),version=editVersion,loadGeneration=editorLoadGeneration,original=JSON.stringify(graph);
  let frame=0,client={x:state.anchorX,y:state.anchorY},position=null,finished=false;
  const valid=()=>graph===owner&&current()===level&&stage===state.stage&&editVersion===version&&editorLoadGeneration===loadGeneration&&!editorMutationBlocked();
  const cancel=()=>{if(finished)return;finished=true;cancelAnimationFrame(frame);controller.abort();layer.remove();nodePlacement=null;$('#connection').hidden=true;};
  const update=()=>{
    if(!valid()){cancel();return;}
    const q=graphPoint(client.x,client.y),startNode=level.nodes.find(n=>n.id===state.wire.node),p=startNode&&point(startNode,state.wire.port,state.wire.kind);
    if(!q||!p){cancel();return;}
    position={x:snap(q.x-size.width/2),y:snap(q.y-size.title/2)};
    outline.style.left=position.x+'px';outline.style.top=position.y+'px';
    const end={x:position.x+size.port.x,y:position.y+size.port.y},a=state.wire.kind==='outputs'?p:end,b=state.wire.kind==='outputs'?end:p,dx=Math.max(60,Math.abs(a.x-b.x)*.5);
    wire.setAttribute('d',`M ${a.x} ${a.y} C ${a.x+dx} ${a.y}, ${b.x-dx} ${b.y}, ${b.x} ${b.y}`);
    const port=$('#cards').querySelector(`[data-node="${CSS.escape(state.wire.node)}"] [data-kind="${state.wire.kind}"][data-port="${CSS.escape(state.wire.port)}"]`);
    wire.style.stroke=getComputedStyle(port).getPropertyValue('--socket').trim()||getComputedStyle(port).borderTopColor;
  };
  const tick=()=>{update();if(!finished)frame=requestAnimationFrame(tick);};
  nodePlacement={cancel};$('#connection').hidden=false;$('#connection').textContent=t('create.place');tick();
  window.addEventListener('pointermove',e=>{client={x:e.clientX,y:e.clientY};},options);
  window.addEventListener('pointerdown',e=>{
    if(!valid()){cancel();return;}
    if(!e.target.closest('#canvas')||e.target.closest('.canvas-view-tools,.graph-navigation,#connectionnotice,.selection-toolbar,#graphtrash')||e.target.closest('button,input,select,textarea,a,[role="button"]')&&!e.target.closest('.node')){cancel();return;}
    e.preventDefault();e.stopImmediatePropagation();
    if(e.button!==0){cancel();return;}
    client={x:e.clientX,y:e.clientY};update();if(finished||!position)return;
    const accept=JSON.stringify(graph)===original,target={...state,...position};cancel();
    if(accept)commitCreatorMatch(match,target,locked);
    // Do not let the trailing click select a covered node or start a wire.
    const release=new AbortController(),releaseOptions={capture:true,once:true,signal:release.signal};
    window.addEventListener('click',event=>{release.abort();event.preventDefault();event.stopImmediatePropagation();},releaseOptions);
    window.addEventListener('pointerup',()=>setTimeout(()=>release.abort(),0),releaseOptions);
    window.addEventListener('pointercancel',()=>release.abort(),releaseOptions);
    window.addEventListener('blur',()=>release.abort(),releaseOptions);
  },options);
  window.addEventListener('keydown',e=>{e.preventDefault();e.stopImmediatePropagation();if(e.key==='Escape'||e.key==='Tab')cancel();},options);
  window.addEventListener('blur',cancel,options);window.addEventListener('resize',cancel,options);
  ...
  document.addEventListener('keydown',e=>{
    ...
    if(['h','f','l'].includes(plainKey)&&!e.ctrlKey&&!e.metaKey&&!e.altKey&&(!e.shiftKey||plainKey==='l')){
      if(graphCommandReady){
        ...
        else if(plainKey==='f'){e.preventDefault();fitSelection();}
