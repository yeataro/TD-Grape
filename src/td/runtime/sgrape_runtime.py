...
def compiled_is_current(comp,compiled,graph=None):
    ...
    if source_module() and graph is not None:
        for decl in graph['declarations']:
            if decl['kind']=='uniform' and not decl.get('sourceMissing') and not source_module().locate(shader_operator(comp),comp.fetch('grapeNativeUniformsV1',{}).get(decl['id'])): return False
